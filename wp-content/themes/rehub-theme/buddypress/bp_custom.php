...
if( ! function_exists( 'rh_nologedin_add_buttons' ) ) :
	function rh_nologedin_add_buttons() {
		...
		?>
			...
			<div class="generic-button">
				<a href="#" title="Send a private message to this user." class="act-rehub-login-popup send-message"><?php echo __( 'Private Message', 'rehub-theme' ); ?></a>
			</div>
			<?php endif;?>
		<?php
		}
...
if( ! function_exists( 'rh_nologedin_add_buttons_group' ) ) :
	function rh_nologedin_add_buttons_group() {
		...
		?>
			<div class="generic-button">
				<a href="#" title="Join Group" rel="add" class="act-rehub-login-popup"><?php echo __( 'Join Group', 'rehub-theme' ); ?></a>
			</div>
		<?php
		}
	}
...
function rh_customize_product_tracking_args() {
    ...
    bp_activity_set_post_type_tracking_args( 'product', array(
        ...
        'bp_activity_admin_filter' => __( 'Published a new product', 'rehub-theme' ),
        'bp_activity_front_filter' => __( 'Products', 'rehub-theme' ),
        'contexts'                 => array( 'activity', 'member' ),
        'activity_comment'         => true,
        'bp_activity_new_post'     => __( '%1$s posted a new <a href="%2$s">product</a>', 'rehub-theme' ),
        'bp_activity_new_post_ms'  => __( '%1$s posted a new <a href="%2$s">product</a>, on the site %3$s', 'rehub-theme' ),
        'position'                 => 100,
    ) );
}
...
function rh_custom_message_placeholder_in_bp_message(){
	if(!empty($_GET['ref'])){
		$content = __('I am interested in: ', 'rehub-theme').urldecode($_GET['ref']);
