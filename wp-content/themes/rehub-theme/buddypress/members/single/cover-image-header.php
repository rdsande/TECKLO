						...
						<div id="rh-header-bp-content">
							...
							<div id="item-meta">	
								<?php if ( bp_is_active( 'activity' ) && bp_activity_do_mentions() ) : ?>			
									<span class="last-activity-profile"><?php _e( 'Last active', 'rehub-theme' );?>: <span><?php bp_last_activity( bp_displayed_user_id() ); ?></span></span>
								<?php endif; ?>
								<?php if ($seo_user_description): ?>
	                                <div class="bp_user_about_details">
	                                    <?php $desc_len = strlen(esc_html($seo_user_description));?>
	                                    <?php if ($desc_len > 180) :?>                           
	                                        <p><?php kama_excerpt('maxchar=180&text='.$seo_user_description); ?> <a href="<?php echo bp_core_get_user_domain($author_ID);?>profile/#item-body"><?php _e('(Read more)', 'rehub-theme');?></a></p>
	                                    <?php else :?>
								...
								<?php _e('SHARE:', 'rehub-theme');?>
								<?php echo rehub_social_inimage('flat', false, false, 'user');?>
								</div>				
							</div>								
						</div>
			            <div id="rh-bp-profile-stats">              
			                <div><?php _e( 'Comments', 'rehub-theme' ); ?>: <span><?php echo (int)$count_comments;?></span></div>
			                <div><?php _e( 'Likes', 'rehub-theme' ); ?>: <span><?php echo (int)$count_p_votes;?></span></div>
			                <div><?php _e( 'Submitted', 'rehub-theme' ); ?>: <span><?php echo (int)$totalsubmitted;?></span></div>
							<?php
							if ( function_exists( 'bp_follow_total_follow_counts' ) ) :?>
							    ...
							    <div><?php _e( 'Followers', 'rehub-theme' ); ?>: <span><?php echo (int)$count['followers']; ?></span></div>
							    <div><?php _e( 'Following', 'rehub-theme' ); ?>: <span><?php echo (int)$count['following']; ?></span></div>
							<?php endif;?>

			                <?php if(bp_is_active( 'friends' )) :?>
				                <div><?php _e( 'Friends', 'rehub-theme' ); ?>: <span><?php echo friends_get_total_friend_count();?></span></div>				 				
			                <?php endif;?> 
			...
			<div class="rh-container">
				<div id="item-nav">
					<div class="responsive-nav-greedy item-list-tabs no-ajax rh-flex-eq-height clearfix" id="object-nav" role="navigation">
						...
						<span class="togglegreedybtn rhhidden floatright ml5"><?php _e('More', 'rehub-theme');?></span>
