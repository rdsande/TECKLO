...
function rehub_enqueue_scripts() {
	...
	$translation_array = array( 
		'back' => __( 'back', 'rehub-theme' ), 
		'ajax_url' => admin_url( 'admin-ajax.php', 'relative' ),
		'templateurl' => get_template_directory_uri(),
		'fin' => __( 'That\'s all', 'rehub-theme' ),
		'your_rating' => __( 'Your Rating:', 'rehub-theme' ),
	...
	$affcoupons_array = array( 
		'coupontextready' => __( 'Here is your coupon code', 'rehub-theme' ), 
		'coupontextcopied' => __( 'Code is copied', 'rehub-theme' ),
		'coupongoto' => __( 'Go to ', 'rehub-theme' ),	
		'couponwebsite' => __( 'Website', 'rehub-theme' ),
		'couponuseoffer' => __( ' and use this Offer.', 'rehub-theme' ),
		'couponorcheck' => __( 'Or check your new window for opened website', 'rehub-theme' ),						  
	);		
...
function rehub_register_my_menus() {
	register_nav_menus(
		array(
			'primary-menu' => __( 'Primary Menu', 'rehub-theme' ),
			'top-menu' => __( 'Top Menu', 'rehub-theme' ),
			'user_logged_in_menu' => __( 'User Logged In Menu', 'rehub-theme' ),
		)
	);
}
}
...
if( ! function_exists( 'rh_custom_sliding_elements' ) ) {
	function rh_custom_sliding_elements(){
		...
		if($slidingtext || $slidinglogo){
			...
			if($slidinglogo){
				$output.= '<div class="text-center"><img id="mobpanelimg" src="'.esc_url($slidinglogo).'" alt="Logo" /></div>';
			}
			if($slidingtext){
...
function wpsm_thumb( $size = 'small', $lazy = true ){
	if( $size == 'medium_news' ){$width = 432; $height = 250; $nothumb = get_template_directory_uri() . '/images/default/noimage_432_250.png' ;}
		...
		if ('post' == $post->post_type) {
			...
			if ( ! empty( $categories ) ) {
			    ...
			    foreach( $categories as $category ) {
			        $output .= '<a class="cat" href="' . esc_url( get_category_link( $category->term_id ) ) . '" title="' . esc_attr( sprintf( __( 'View all posts in %s', 'rehub-theme' ), $category->name ) ) . '">' . esc_html( $category->name ) . '</a>' . $separator;
			    }
...
function rh_post_header_meta ($admin_exist = true, $time_exist = true, $view_exist = true, $comment_exist = true, $cats_post = true ){
	...
	if(rehub_option('exclude_comments_meta') != 1 && ($comment_exist != false)){ ?>
		<?php if($comment_exist=='compact'):?>
			<span class="comm_count_meta"><?php comments_popup_link( __('0','rehub-theme'), __('1','rehub-theme'), __('%','rehub-theme'), 'comm_meta', ''); ?></span>
		<?php elseif ($comment_exist == 'compactnoempty'):?>
			<?php if($post->comment_count > 0):?>
			<span class="comm_count_meta"><?php comments_popup_link( __('0','rehub-theme'), __('1','rehub-theme'), __('%','rehub-theme'), 'comm_meta', ''); ?></span>
			<?php endif;?>
		<?php else:?>
			<span class="comm_count_meta"><?php comments_popup_link( __('no comments','rehub-theme'), __('1 comment','rehub-theme'), __('% comments','rehub-theme'), 'comm_meta', ''); ?></span>
		<?php endif;?>
	<?php }	
	if(rehub_option('exclude_cat_meta') != 1 && ($cats_post != false)){ 
		$postidforcat = $post->ID;
	...
	?>
		...
		<div class="floatright ml15 postviewcomm mt5">
			...
			<?php if(rehub_option('exclude_comments_meta') != 1):?>			
				<span class="comm_count_meta"><strong><?php comments_popup_link( __('0','rehub-theme'), __('1 comment','rehub-theme'), __('% comments','rehub-theme'), 'comm_meta', ''); ?></strong></span>	
			<?php endif;?>			
		</div>	
	<?php				 	
}
		...
		if ($post_type=='post' && 'post' == get_post_type($post_id)) {
			...
			if ( ! empty( $categories ) ) {
			    foreach( $categories as $category ) {
			        ...
			        $output .= '<a class="rh-cat-label-title rh-cat-'.$category->term_id.'" href="' . esc_url( get_category_link( $category->term_id ) ) . '" title="' . esc_attr( sprintf( __( 'View all posts in %s', 'rehub-theme' ), $category->name ) ) . '">' . esc_html( $category->name ) . '</a>' . $separator;
			    }
			    echo trim( $output, $separator );
			}
			if(rehub_option('enable_brand_taxonomy') == 1 && $dealstore == true){ 
				...
				if( ! empty( $dealcats ) && ! is_wp_error( $dealcats ) ) {
					foreach( $dealcats as $dealcat ) {
				        echo '<a class="rh-cat-label-title rh-dealstore-cat rh-cat-'.$dealcat->term_id.'" href="' . esc_url( get_term_link( $dealcat->term_id ) ) . '" title="' . esc_attr( sprintf( __( 'View all posts in %s', 'rehub-theme' ), $dealcat->name ) ) . '">' . esc_html( $dealcat->name ) . '</a>' . $separator;						
					}
				}								
			}		
		}
...
function meta_small ($time_exist, $cats_exist, $comment_exist, $post_views = false ){
	...
	if(rehub_option('exclude_comments_meta') != 1 && ($comment_exist != false)){ ?>
		...
		<?php endif;?>
	<?php } 
	if($post_views != false && rehub_option('post_view_disable') != 1){ ?>
		<?php $rehub_views = get_post_meta (get_the_ID(),'rehub_views',true); if ($rehub_views !='') :?>
			<span class="postview_meta"><?php echo (int)$rehub_views; ?> </span>
		<?php endif ;?>
	<?php }     	   	
	...
	<li <?php comment_class(); ?> id="comment-<?php comment_ID() ?>">
		<div class="commbox">
			<div class="comment-author vcard clearfix">
				...
				<?php comment_reply_link(array_merge( $args, array('reply_text' => __(' Reply', 'rehub-theme'), 'depth' => $depth, 'max_depth' => $args['max_depth'])), $comment->comment_ID); ?>                    
...
function my_theme_register_required_plugins() {
    ...
    $config = array(
        ...
        'strings'      => array(
            'page_title'                      => __( 'Install Required Plugins', 'rehub-theme' ),
            'menu_title'                      => __( 'Install Plugins', 'rehub-theme' ),
            'installing'                      => __( 'Installing Plugin: %s', 'rehub-theme' ), // %s = plugin name.
            'oops'                            => __( 'Something went wrong with the plugin API.', 'rehub-theme' ),
            ...
            'return'                          => __( 'Return to Required Plugins Installer and Activate plugins', 'rehub-theme' ),
            'plugin_activated'                => __( 'Plugin activated successfully.', 'rehub-theme' ),
            'complete'                        => __( 'All plugins installed and activated successfully. %s', 'rehub-theme' ), // %s = dashboard link.
