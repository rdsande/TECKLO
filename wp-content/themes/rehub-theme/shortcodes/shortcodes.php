			        ...
			        <div class="flowhidden pb10 pt15 border-grey-bottom">               
			            ...
			            <div class="floatright buttons_col pl20 rtlpr20 wpsm-one-half-mobile wpsm-column-last">
		                    ...
		                    <?php if($this_id != $currentid):?>
								<a href="<?php the_permalink();?>" class="font80 lineheight15 clearbox floatright"><?php _e('Details', 'rehub-theme');?></a>
							<?php endif; ?>	                                 
			            </div>                                  
			            ...
			            <div class="rh_listofferitem rh_list_mbl_im_left border-grey-bottom"> 
			            	<div class="rh-flex-center-align rh-flex-justify-center pt15 pb15 mobileblockdisplay">
				                ...
				                <div class="text-right-align rh_listcolumn_btn pr15 rtlpl15">
				                    ...
				                    <?php if($this_id != $currentid):?>
										<a href="<?php the_permalink();?>" class="font80 details-link-list"><?php _e('Details', 'rehub-theme');?></a>	
									<?php endif; ?>	                    
				                </div>
			                </div>                                                                         
			            </div>               					
...
function rehub_affbtn_function( $atts, $content = null ) { 
    ...
    if ($meta_btn_url || $btn_url) :
		...
		elseif (rehub_option('rehub_btn_text') !='') :
			...
			$out .= __("Buy this item", "rehub-theme");	
		endif;
		$out .='</a></div></div>';                	
	else :	
    	ob_start();
...
if(class_exists('Woocommerce') && !empty($product_id)){
	...
	$title_label = (!empty($atts['title'])) ? $atts['title'] : __('Specification', 'rehub-theme');
...
function wpsm_toprating_shortcode( $atts, $content = null ) {
	...
	if(isset($atts['id']) || isset($atts['postid'])):
    	...
    	?>
            ...
            <?php while ($wp_query->have_posts()) : $wp_query->the_post(); global $post; $i ++?>
                ...
                <div class="rh-flex-center-align rh-flex-justify-center pt15 pb15 mobileblockdisplay">                   
	                ...
	                <div class="listbuild_btn listitem_column text-center">
		            	...
		            	<?php endif;?>                 
	                    <a href="<?php the_permalink();?>" class="read_full font70"><?php if(rehub_option('rehub_review_text') !='') :?><?php echo rehub_option('rehub_review_text') ; ?><?php else :?><?php _e('Read full review', 'rehub-theme'); ?><?php endif ;?></a>
	                </div>
	            </div>
                </div>
				<?php if($disclaimer):?>
            ...
            <?php wp_reset_query(); ?>
            <?php else: ?><?php _e('No posts for this criteria.', 'rehub-theme'); ?>
            <?php endif; ?>
...
function wpsm_toptable_shortcode( $atts, $content = null ) {
	...
	if(isset($atts['id']) && $atts['id']):
	    ...
	    $first_column_name = (get_post_meta( $toppost->ID, 'first_column_name', true ) !='') ? esc_html(get_post_meta( $toppost->ID, 'first_column_name', true )) : __('Product', 'rehub-theme') ;
...
function wpsm_woocharts_shortcode( $atts, $content = null ) {
	...
	if(!empty($compareids)):
		    ...
		    <div class="top_chart table_view_charts loading">		        	
                ...
                <div class="top_chart_first">
                    <ul>
                        <li class="row_chart_0 image_row_chart">
                            <div class="sticky-cell"><br /><label class="diff-label"><input class="re-compare-show-diff" name="re-compare-show-diff" type="checkbox" /><?php _e('Show only differences', 'rehub-theme');?></label></div>
                        </li>
                        <li class="row_chart_1 heading_row_chart">
                            <?php _e('Overview', 'rehub-theme');?>
                        </li>                        
                        <li class="row_chart_2 meta_value_row_chart">
                            <?php _e('Description', 'rehub-theme');?>
                        </li>                          
                        <li class="row_chart_5 meta_value_row_chart">
                            <?php _e('Brand/Store', 'rehub-theme');?>
                        </li>                                                 
                        <li class="row_chart_7 meta_value_row_chart">
                            <?php _e('Availability', 'rehub-theme');?>
                        </li> 
                        <li class="row_chart_3 meta_value_row_chart">
                            <?php _e('User Rating', 'rehub-theme');?>
                        </li>                         
                        <?php if ($common_criterias):?>
                        <li class="row_chart_criterias row_chart_6">
                            <?php _e('Review', 'rehub-theme');?>
                        </li> 
                        <?php endif;?>                          
                        <?php if(!empty($common_attributes)):?>
	                        <li class="row_chart_8 heading_row_chart">
	                            <?php _e('Attributes', 'rehub-theme');?>
	                        </li>                        
		    	...
		    	<div class="top_chart_wrap woocommerce"><div class="top_chart_carousel">
			        <?php while ($wp_query->have_posts()) : $wp_query->the_post(); global $product, $post; $ci ++?>
			            <div class="top_rating_item top_chart_item compare-item-<?php echo (int)$post->ID;?>" id='rank_<?php echo (int)$i?>' data-compareid="<?php echo (int)$post->ID;?>">
			                <ul>
                                <li class="row_chart_0 image_row_chart">
                                    <div class="product_image_col sticky-cell">                                  
                                        ...
                                        <figure>
								            <?php if ( $product->is_featured() ) : ?>
								                    <?php echo apply_filters( 'woocommerce_featured_flash', '<span class="onfeatured">' . __( 'Featured!', 'rehub-theme' ) . '</span>', $post, $product ); ?>
								            <?php endif; ?>        
                                        ...
                                        <div class="price-in-compare-flip mt20">
										    ...
										    <?php if (defined('\ContentEgg\PLUGIN_PATH')):?>
										        ...
										        <?php endif;?>
										    <?php endif;?>                                            
							                <?php if ( $product->is_in_stock() && $product->add_to_cart_url() !='' && $syncitem=='') : ?>
							                    <?php  echo apply_filters( 'woocommerce_loop_add_to_cart_link',
                                ...
                                <li class="row_chart_7 meta_value_row_chart">
                                	<?php if ( $product->is_in_stock() ):?>
										<span class="greencolor"><?php _e( 'In stock', 'rehub-theme' ) ;?></span>
									<?php else :?>
										<span class="redcolor"><?php _e( 'Out of stock', 'rehub-theme' ) ;?></span>
									<?php endif;?>
                                </li>
                                <li class="row_chart_3 meta_value_row_chart">
		...
		$output = __('No products for comparison', 'rehub-theme');
		return $output;
	endif;	
...
function wpsm_scorebox_shortcode( $atts, $content = null ) {
	...
	<?php else :?>   
    	<?php $title = __('Average Score', 'rehub-theme');?>
    <?php endif ;?>
...
if (is_user_logged_in()) {
	...
	if ( function_exists('bp_notifications_get_notifications_for_user')) {
		...
		$notification_bp_item .='<li class="bp-profile-edit-menu-item menu-item"><a href="'.bp_core_get_user_domain( $user_id ).'"><i class="fas fa-cogs"></i></i><span>'. __("Edit Profile", "rehub-theme") .'</span></a></li>';		
    ...
    $output .= '<ul class="user-dropdown-intop-menu">';
        ...
        $output .= '</li>';
        if ($profile_url) :
        	$output .= '<li class="user-profile-link-intop menu-item"><a href="'. esc_url(get_the_permalink($profile_url)) .'"><i class="fas fa-user-circle"></i><span>'. __("My profile", "rehub-theme") .'</span></a></li>';
        endif;
        if ($submit_url) :
        	...
        	if(!$submit_url_label){
        		$submit_url_label = __("Submit a Post", "rehub-theme");
        	}
        ...
        if ($edit_url) :
        	...
        	if(!$edit_url_label){
        		$edit_url_label = __("Edit My Posts", "rehub-theme");
        	}        	     	
        ...
        if (defined('wcv_plugin_dir')) :
        	...
        	if (!empty($redirect_to)){
	        	$output .= '<li class="user-editshop-link-intop menu-item"><a href="'. esc_url($redirect_to) .'"><i class="fas fa-shopping-bag" aria-hidden="true"></i><span>'. __("Manage Your Shop", "rehub-theme") .'</span></a></li>';
	        	$ifvendor = true;        	
        	}
        endif; 
		if( defined( 'WCFMmp_TOKEN' ) && wcfm_is_vendor( $user_id ) ):
			$output .= '<li class="user-editshop-link-intop menu-item"><a href="'. esc_url(get_wcfm_url()) .'"><i class="fas fa-shopping-bag" aria-hidden="true"></i><span>'. __("Manage Your Shop", "rehub-theme") .'</span></a></li>'; 
			$ifvendor = true;
		endif;        
        if( function_exists('dokan_is_user_seller')) :
        	$is_vendor = dokan_is_user_seller( $user_id );
        	if($is_vendor) :
	        	$output .= '<li class="user-editshop-link-intop menu-item"><a href="'. dokan_get_navigation_url() .'"><i class="fas fa-shopping-bag" aria-hidden="true"></i><span>'. __("Manage Your Shop", "rehub-theme") .'</span></a></li>'; 
	        	$ifvendor = true;
	        endif; 
        endif; 
        if( class_exists('WCMp')) :
        	...
        	if($is_vendor) :
        		...
        		if ($dashlink > 0):
	        		$output .= '<li class="user-editshop-link-intop menu-item"><a href="'. get_permalink($dashlink) .'"><i class="fas fa-shopping-bag" aria-hidden="true"></i><span>'. __("Manage Your Shop", "rehub-theme") .'</span></a></li>'; 
	        	endif; 
	        	$ifvendor = true;        		
    		endif;
        endif; 
        if(class_exists('Woocommerce') && $ifvendor ==false && rehub_option('disable_woo_scripts') != 1){
        	...
        	if(!empty($myaccpageid)){
	        	$output .= '<li class="user-editorders-link-intop menu-item"><a href="'. get_permalink($myaccpageid) .'"><i class="fas fa-shopping-bag" aria-hidden="true"></i><span>'. __("Manage Your Orders", "rehub-theme") .'</span></a></li>';
        	}         	
        }                                 
        ...
        $output .= '<li class="user-logout-link-intop menu-item"><a href="'. wp_logout_url( home_url()) .'"><i class="far fa-lock-alt"></i><span>'. __("Log out", "rehub-theme") .'</span></a></li>';
$output .= '</ul></div>';
} else {
...
function wpsm_comparison_button( $atts, $content = null ) {
        $atts = shortcode_atts(
			array(
				...
				'id' => '',
				'woocats' => '',
				'label' => __("Add to compare", "rehub-theme"),
			), $atts);
...
function wpsm_get_custom_value($atts){
    ...
    if($result){
    	if($label) {$out .='<div class="rh-flex-center-align">';}
	    if($list){
...
function wpsm_tax_archive_shortcode( $atts, $content = null ) {
	...
	if($type == 'alpha') {
		if( ! empty( $terms ) && ! is_wp_error( $terms ) ) {
			...
			foreach( $letter_keyed_terms as $letter => $terms ) {
				...
				$term_titles .= '<div class="single-letter mt20 mb20 pb10 border-grey-bottom"><a href="#" name="'.$letter.'"></a><div class="letter_tag fontbold font120 lineheight20">'.$letter.'<div class="return_to_letters cursorpointer floatright font80"><span class="rehub_scroll rehub-main-color-bg" data-scrollto="#top_ankor"><i class="far fa-angle-up"></i></span></div></div></div>';
				...
				foreach( $terms as $term ) {
					...
					$term_titles .= '<a class="single-letter-link rh-flex-column rh-flex-center-align rh-flex-justify-center pt10 pb10 pl10 pr10 floatleft mt10 mr10 text-center" id="taxonomy-'. $term->term_id .'"  href="' . esc_url( get_term_link( $term ) ) . '" title="' . esc_attr( sprintf( __( 'View all post filed under %s', 'rehub-theme' ), $term->name ) ) . '">' . $thumbnail . '<h5 class="mt5 mb0 font80 fontnormal">'. $term->name . '</h5></a>';
				}
	...
	elseif ($type == 'compact') {
		if( ! empty( $terms ) && ! is_wp_error( $terms ) ) {
			foreach( $terms as $term ) {
				$term_titles .= '<a class="'.$classitem.' compact-tax-link tax-item text-center floatleft mt10 mr10" href="' . esc_url( get_term_link( $term ) ) . '" title="' . esc_attr( sprintf( __( 'View all post filed under %s', 'rehub-theme' ), $term->name ) ) . '"><h5 class="mt0 mb0 font80 lineheight15">'.esc_html($anchor_before).$term->name.esc_html($anchor_after).'</h5></a>';
			}
	...
	elseif ($type == 'inlinelinks') {
		if( ! empty( $terms ) && ! is_wp_error( $terms ) ) {
			foreach( $terms as $term ) {
				$term_titles .= '<span class="inlinestyle position-relative pt10 pb10 pl15 pr15 '.$classitem.'"><a class="" href="' . esc_url( get_term_link( $term ) ) . '" title="' . esc_attr( sprintf( __( 'View all post filed under %s', 'rehub-theme' ), $term->name ) ) . '"><span class="blockstyle">'.esc_html($anchor_before).$term->name.esc_html($anchor_after).'</span><span class="blockstyle rh_opacity_7 font80">'.$term->count.__(' Products', 'rehub-theme').'</span></a></span>';
			}
	...
	elseif ($type == 'compactbig') {
		if( ! empty( $terms ) && ! is_wp_error( $terms ) ) {
			...
			foreach( $terms as $term ) {
				$term_titles .= '<a class="'.$classitem.' col_item mb10 text-center rh-main-bg-hover blackcolor rh-cartbox big-tax-link" href="' . esc_url( get_term_link( $term ) ) . '" title="' . esc_attr( sprintf( __( 'View all post filed under %s', 'rehub-theme' ), $term->name ) ) . '"><div class="rehub-main-font">'.esc_html($anchor_before).$term->name.esc_html($anchor_after).'</div></a>';
			}
	...
	elseif ($type == 'logo') {
		if( ! empty( $terms ) && ! is_wp_error( $terms ) ) {
			foreach( $terms as $term ) {
				...
				if ($thumbnail){
					$term_titles .= '<a class="'.$classitem.' col_item mb10 rh_centered_image text-center rh-cartbox pt10 pb10 pl10 pr10 logo-tax-link" href="' . esc_url( get_term_link( $term ) ) . '" title="' . esc_attr( sprintf( __( 'View all post filed under %s', 'rehub-theme' ), $term->name ) ) . '">'. $thumbnail . '</a>';					
				}
			}
			return '<div class="'.$classcol.' rh-flex-eq-height">'.$term_titles.'</div>';	
		}
		...
		if( ! empty( $terms ) && ! is_wp_error( $terms ) ) {
			foreach( $terms as $term ) {
				...
				if( $term->parent == '0' ){
					...
					$term_titles .= '<div '. $right_style .'><h5 class="rh-heading-hover-color blockstyle mb10 fontnormal font110 mt0"><a class="" href="'. esc_url(get_term_link($term)) .'" title="'. esc_attr(sprintf( __('View all post filed under %s', 'rehub-theme' ), $term->name)) .'">'. esc_html($term->name) . '</a></h5>';
					$term_titles .= '<div class="subcategortes font70 lineheight15 blackcolor">'. $term_childes .''. $ellipsis .'</div></div>'; 
					$term_titles .= ($thumbnail_url) ? '<a href="'. esc_url(get_term_link($term)) .'" title="'. esc_attr(sprintf( __('View all post filed under %s', 'rehub-theme' ), $term->name)) .'" class="rh-flex-right-align"><img src="'. esc_url($thumbnail_url) .'" alt="'. esc_attr($term->name) .'" width="'. $imageheight .'" /></a>' : '';					
					$term_titles .= '</div>';
				}
			}
		...
		if(!empty($terms) && !is_wp_error($terms)) {
			...
			foreach( $terms as $term ) {
				...
				if( $show_images == 1 ) {
					...
					if ($thumbnail_url) {
						$thumbnail = '<img src="'. esc_url($thumbnail_url) .'" alt="'. esc_attr($term->name) .'" />';
					}
				}
				if($thumbnail){
...
function wpsm_update_shortcode( $atts, $content = null ) {
	...
	$label = (!empty($atts['label'])) ? $atts['label'] : __('Update', 'rehub-theme');
	$content = do_shortcode($content);	
...
function rh_wcv_vendorslist_flat( $atts ) {
		...
		<div class="tabledisplay mb20">
			<form id="search-sellers" role="search" method="get" class="celldisplay search-form floatleft mb10">
				<input type="text" name="search_sellers" placeholder="'. __('Search sellers', 'rehub-theme') .'" value="">
				<button type="submit" alt="'. __('Search', 'rehub-theme') .'" value="'. __('Search', 'rehub-theme') .'" class="btnsearch"><i class="fal fa-search"></i></button>
			</form>
			<form id="filter-sellers" method="get" class="celldisplay floatright mb10 ml10">
				<label>'. __('Sort by:', 'rehub-theme') .'</label>
				<select name="orderby_sellers" class="orderby">
					<option value="alphabet"'. $alphabet .'>'. __('Alphabetical', 'rehub-theme') .'</option>
					<option value="mostpopular"'. $mostpopular .'>'. __('Most popular', 'rehub-theme') .'</option>
					<option value="mostresent"'. $mostresent .'>'. __('Most recent', 'rehub-theme') .'</option>
				</select>
			</form>
	    ...
	    foreach ($paged_vendors as $vendor) {
			...
			elseif ( class_exists( 'WeDevs_Dokan' ) ){
            	...
            	$shop_name = isset( $store_info['store_name'] ) ? esc_html( $store_info['store_name'] ) : __( 'Noname Shop', 'rehub-theme' );	    				
			}
			elseif ( defined( 'WCFMmp_TOKEN' ) ){
...
function wpsm_get_add_deal_popup($atts, $content = NULL){
	...
	if($post_id && is_numeric($post_id)){
		...
		$rand = mt_rand();
		?>		
		<a class="btn_offer_block csspopuptrigger rh-deal-compact-btn act-rehub-addoffer-popup act-rehub-login-popup" data-popup="addfrontdeal_<?php echo ''.$rand;?>"><?php _e('Add your deal', 'rehub-theme') ?></a>
			...
			<div class="csspopup" id="addfrontdeal_<?php echo ''.$rand;?>">
				<div class="csspopupinner addfrontdeal-popup">
					...
					<?php if ( in_array( $role, (array) $current_user->roles )):?>
						...
						<?php if (false === in_array($current_user->ID, $multioffer_names)):?>
							<div class="rehub-offer-popup">
								<div class="re_title_inmodal"><?php _e('Add an Offer', 'rehub-theme'); ?></div>
								<form id="rehub_add_offer_form_modal" action="<?php echo home_url( '/' ); ?>" method="post">
									<div class="re-form-group mb20">
										<label for="rehub_product_name"><?php _e('Name of product', 'rehub-theme') ?><span>*</span></label>
										<input class="re-form-input required" name="rehub_product_name" id="rehub_product_name" type="text" />
									</div>
									<div class="re-form-group mb20">
										<label for="rehub_product_url"><?php _e('Offer url', 'rehub-theme') ?><span>*</span></label>
										<input class="re-form-input required" name="rehub_product_url" id="rehub_product_url" type="url" required />
									</div>
									<div class="re-form-group mb20">
										<label for="rehub_product_price"><?php _e('Offer sale price (example, $55)', 'rehub-theme') ?><span>*</span></label>
										<input class="re-form-input required" name="rehub_product_price" id="rehub_product_price" type="text" />
									</div>
									<div class="re-form-group mb20">
										<label for="rehub_product_desc"><?php _e('Short description', 'rehub-theme') ?><span></span></label>
									...
									<div class="re-form-group mb20">
										...
										<button class="wpsm-button rehub_main_btn" type="submit" name="send"><?php _e('Send', 'rehub-theme'); ?></button>
									</div>
								</form>
								<div class="rehub-errors"></div>
							</div>
						<?php else:?>
							<?php _e('You already added your deal to this post', 'rehub-theme');?>
						<?php endif;?>
					<?php else:?>
					...
					<div class="rehub-offer-popup-ok font110 rhhidden">
						<div class="re_title_inmodal"><?php _e('Send Offer', 'rehub-theme'); ?></div>
							<?php printf( __('<strong>Thank you, %s!</strong> Your offer has been sent', 'rehub-theme'), $current_user->display_name ); ?>
					</div>
				</div>				
			</div>
		<?php endif;?>
...
function rh_get_group_admins($atts, $content = NULL){
	...
	$txt = ($text) ? $text : __('Write message', 'rehub-theme') ;
	if ( ! empty( $group->admins ) ) { 
		$output .= '<ul class="buddypress widget">';
			foreach( (array) $group->admins as $admin ) {
				$output .= '<li class="vcard mb15">';
					$output .= '<div class="item-avatar"><a href="'.bp_core_get_user_domain( $admin->user_id, $admin->user_nicename, $admin->user_login ).'">'.bp_core_fetch_avatar( array( 'item_id' => $admin->user_id, 'email' => $admin->user_email, 'alt' => sprintf( __( 'Profile picture of %s', 'rehub-theme' ), bp_core_get_user_displayname( $admin->user_id ) ) ) ).'</a></div>';
...
function rh_ce_search_form( $atts=array(), $content = null ) {
	$build_args =shortcode_atts(array(
		'placeholder' => __('Search Products...', 'rehub-theme'),
		'label' => __('Search', 'rehub-theme'),		
...
function wpsm_reviewbox( $atts, $content = null ) {
        $atts = shortcode_atts(
			array(
				...
				'score' => '',
				'pros' => '',
				'prostitle' => __('PROS', 'rehub-theme'),				
				'cons' => '',
				'constitle' => __('CONS', 'rehub-theme'),
				'id' => '',
				'compact' =>'',
				'scrollid' => '',
				'woobox' => '',
    ...
    if($compact){
    	...
    	if($score){
    		...
    		elseif($compact == 'squarecolor'){
		    	...
		    	$title = (!empty($title)) ? $title : __('Expert Score', 'rehub-theme');
		    	$description = (!empty($description)) ? $description : __('Read review', 'rehub-theme');
	...
	elseif($regular){
		...
		$out = '<div class="rate_bar_wrap"><div class="review-top">';
			...
			$out .= $headinghtml.$description.'</div></div></div><div class="rh-line mb10 mt10"></div>';	
			if (!empty($thecriteria))  {
					...
					<div class="commwrap floatleft" style="width:calc(100% - <?php echo (int)$img_width_2; ?>px)">
						...
						<?php 
							if( isset( $userAverage ) && $userAverage != '' ) {
								...
								echo '<div class="user_reviews_view_score mb0"><div class="userstar-rating" title="'. __('Rated', 'rehub-theme') .' '. $userstartitle .' '. __('out of', 'rehub-theme') .' 5"><span style="width:'. $userAverages .'%"><strong class="rating">'. $userstartitle .'</strong></span></div></div>';
							}
							printf( __( 'Reviewed on <span class="date greycolor">%s</span> by', 'rehub-theme' ), get_comment_date( get_option( 'date_format' ), $comment_ID ) );
