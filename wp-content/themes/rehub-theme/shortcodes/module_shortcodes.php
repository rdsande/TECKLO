...
function rehub_custom_taxonomy_dropdown( $taxdrop, $limit = '40', $class, $taxdroplabel = '', $containerid ='', $taxdropids = '' ) {
	...
	if ( $terms && !is_wp_error($terms) ) {
		...
		if (empty($taxdroplabel)){$taxdroplabel = __('Choose category', 'rehub-theme');}
			...
			<div class="col-feat-grid item-<?php echo ''.$col_number;?>" style="background-image: url('<?php echo esc_url($image_url) ;?>');">
				<a href="<?php the_permalink();?>" class="feat_overlay_link"></a> 
		  		<div class="feat-grid-overlay text_in_thumb">
		    		...
		    		<?php if ($col_number == 1) :?>
		    		<div class="post-meta">
                		<span class="date_ago"><i class="far fa-clock"></i> <?php printf( __( '%s ago', 'rehub-theme' ), human_time_diff( get_the_time( 'U' ), current_time( 'timestamp' ) ) ); ?></span>		    		
	...
	$i=1; if ( $wp_query->have_posts() ) : ?>
		...
		<div class="rh-flex-eq-height products <?php echo ''.$infinitescrollwrap; echo ''.$col_wrap.$gridtypeclass; ?>" data-filterargs='<?php echo ''.$jsonargs.'';?>' data-template="<?php echo ''.$gridtypetemplate;?>" id="<?php echo esc_attr($containerid);?>" data-innerargs='<?php echo ''.$json_innerargs.'';?>'>                   
			...
			<?php elseif ($enable_pagination == '2' || $enable_pagination == '3' ) :?> 
			    <div class="re_ajax_pagination"><span data-offset="<?php echo esc_attr($ajaxoffset);?>" data-containerid="<?php echo esc_attr($containerid);?>" class="re_ajax_pagination_btn def_btn"><?php _e('Show next', 'rehub-theme') ?></span></div>      
			<?php endif;?>
		</div> 
		</div>
	<?php endif; 
    ...
    <div class="left_news_col">
    	<div class="news_out_thumb">
	    	<figure id="n_with_t_<?php echo ''.$rand_id;?>">
	    		<?php if ($color_cat) :?>
		        	<style scoped>
		        		#n_with_t_<?php echo ''.$rand_id;?> .news_cat a{background-color: <?php echo esc_html($color_cat);?>}
		        	</style>
	        	<?php endif;?>
...
<?php if ($style == 2):?>  
    <div class="loading carousel-style-fullpost <?php echo ''.$disable_nav_class;?>">
        <div class="re_carousel eq_grid carousel-style-3 post_eq_grid" data-showrow="<?php echo esc_attr($showrow);?>" <?php echo ''.$autodata;?> <?php echo ''.$disable_nav;?> data-laizy="0">
...
function wpsm_offerbox_shortcode( $atts, $content = null ) {
	...
	if ($offer_coupon_mask_text =='') {
		if(rehub_option('rehub_mask_text') !=''){
			...
			$offer_coupon_mask_text = __('Reveal', 'rehub-theme');
		}
	}
	...
	if ($button_text =='') {
		if(rehub_option('rehub_btn_text') !=''){
			...
			$button_text = __('Buy this item', 'rehub-theme');
		}
	} 
