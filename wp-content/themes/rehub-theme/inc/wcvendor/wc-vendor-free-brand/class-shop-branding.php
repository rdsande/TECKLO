...
class RH_WCVendors_Free_Branding {
	...
	public function admin_user_meta_branding( $user ) {
		...
		?>
			<!-- Shop Phone -->
			<tr>
				<th><label for="_pv_shop_phone"><?php _e( 'Shop Phone', 'rehub-theme' ); ?></label></th>
				<td>
			...
			<!-- Shop Address -->
			<tr>
				<th><label for="_pv_shop_address"><?php _e( 'Shop Contact Address', 'rehub-theme' ); ?></label></th>
				<td>
			...
			<!-- Shop Banner -->
			<tr>
				<th><label for="_pv_shop_banner_id"><?php _e( 'Shop Banner', 'rehub-theme' ); ?></label></th>
				<td>
					<div class="wcv-file-uploader_pv_shop_banner_id">
						<?php if ( $has_banner ) : ?>
							<img src="<?php echo esc_url($banner_src[0]); ?>" alt="image" style="max-width:100%;" /><?php else: ?><?php _e( 'Upload an image for the banner.', 'rehub-theme' ); ?>
						<?php endif; ?>
					</div>
					
					<input id="_wcv_add_pv_shop_banner_id" type="button" class="button" value="<?php _e( 'Add Banner', 'rehub-theme' ); ?>" />
					<input id="_wcv_remove_pv_shop_banner_id" type="button" class="button" value="<?php _e( 'Remove Banner', 'rehub-theme' ); ?>" />
					<input type="hidden" name="_pv_shop_banner_id" id="_pv_shop_banner_id" data-save_button="<?php _e( 'Add Banner', 'rehub-theme' ); ?>" data-window_title="<?php _e( 'Add Banner', 'rehub-theme' ); ?>" data-upload_notice="<?php _e('Upload an image for the banner.', 'rehub-theme' ); ?>" value="<?php echo ''.$banner_id; ?>">
				</td>
			</tr>
			<!-- Shop Icon -->
			<tr>
				<th><label for="_pv_shop_icon_id"><?php _e( 'Shop Icon', 'rehub-theme' ); ?></label></th>
				<td>
					<div class="wcv-file-uploader_pv_shop_icon_id">
						<?php if ( $has_icon ) : ?>
							<img src="<?php echo esc_url($icon_src[0]); ?>" alt="image" style="max-width:100%;" /><?php else: ?><?php _e( 'Upload an image for the shop icon.', 'rehub-theme' ); ?>
						<?php endif; ?>
					</div>
					
					<input id="_wcv_add_pv_shop_icon_id" type="button" class="button" value="<?php _e( 'Add Icon', 'rehub-theme' ); ?>" />
					<input id="_wcv_remove_pv_shop_icon_id" type="button" class="button" value="<?php _e( 'Remove Icon', 'rehub-theme' ); ?>" />
	...
	public static function file_uploader_helper( $field ) { 
		...
		$field[ 'header_text' ] = isset( $field[ 'header_text' ] ) ? $field[ 'header_text' ] : __('Image', 'rehub-theme' ); 
		$field[ 'add_text' ] = isset( $field[ 'add_text' ] ) ? $field[ 'add_text' ] : __('Add Image', 'rehub-theme' ); 
		$field[ 'remove_text' ] = isset( $field[ 'remove_text' ] ) ? $field[ 'remove_text' ] : __('Remove Image', 'rehub-theme' ); 
		$field[ 'image_meta_key' ] = isset( $field[ 'image_meta_key' ] ) ? $field[ 'image_meta_key' ] 	: '_wcv_image_id';  
		$field[ 'save_button' ] = isset( $field[ 'save_button' ] ) ? $field[ 'save_button' ] : __('Add Image', 'rehub-theme' ); 
		$field[ 'window_title' ] = isset( $field[ 'window_title' ] ) ? $field[ 'window_title' ] : __('Select an Image', 'rehub-theme' ); 
	...
	public static function store_banner() {
		...
		echo '<h6>'. __( 'Store Banner', 'rehub-theme'). '</h6>'; 
		echo '<span>'. __('Recommended image height: 270px','rehub-theme') .'</span>';
		self::file_uploader_helper( apply_filters( 'wcv_vendor_store_banner', array(  
			'header_text' => __('Store Banner', 'rehub-theme' ), 
			'add_text' => __('Add Store Banner', 'rehub-theme' ), 
			'remove_text' => __('Remove Store Banner', 'rehub-theme' ), 
			'image_meta_key' => '_pv_shop_banner_id', 
			'save_button' => __('Add Store Banner', 'rehub-theme' ), 
			'window_title' => __('Select an Image', 'rehub-theme' ), 
			'value' => $value
			)
		) );
		echo '</div>';
	...
	public static function store_icon() {
		...
		echo '<h6>'. __( 'Store Icon', 'rehub-theme'). '</h6>';
		echo '<span>'. __('Minimal image size: 150px x 150px','rehub-theme') .'</span>';
		self::file_uploader_helper( apply_filters( 'wcv_vendor_store_icon', array(  
			'header_text' => __('Store Icon', 'rehub-theme' ), 
			'add_text' => __('Add Store Icon', 'rehub-theme' ), 
			'remove_text' => __('Remove Store Icon', 'rehub-theme' ), 
			'image_meta_key' => '_pv_shop_icon_id', 
			'save_button' => __('Add Store Icon', 'rehub-theme' ), 
			'window_title' => __('Select an Image', 'rehub-theme' ), 
			'value' => $value, 
			'size' => 'thumbnail', 
	...
	public static function store_contact() {
		...
		$phone .= __( 'Shop Phone', 'rehub-theme' );
		$phone .= '</h6>';
		$phone .= __( 'Your shop contact phone number.', 'rehub-theme' );
		$phone .= '<br><input type="text" name="_pv_shop_phone" id="_pv_shop_phone" style="width:100%" placeholder="'. __( 'Phone number', 'rehub-theme' ) .'" value="'. $phone_str .'">';
		$phone .= '	</p></div>';
		
		$address = '<div class="pv_shop_phone_container"><h6>';
		$address .= __( 'Shop Address', 'rehub-theme' );
		$address .= '</h6>';
		$address .= __( 'Address where your store is placed.', 'rehub-theme' );
		$address .= '<br><input type="text" name="_pv_shop_address" id="_pv_shop_address" style="width:100%" placeholder="'. __( 'Address', 'rehub-theme' ) .'" value="'. $address_str .'">';
		$address .= '	</p></div>';
