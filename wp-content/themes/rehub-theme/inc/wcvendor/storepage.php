	    			...
	    			<div class="wcvendor_store_desc">
					    <?php if ( class_exists( 'WCVendors_Pro' ) ) :?>
							...
							<?php if ($address) : ?>
								...
								<span class="rehub_scroll" data-scrollto="#wcvendors_pro_map_widget"><?php _e( '(Show on map)', 'rehub-theme' ); ?></span>
								<?php } ?>
							<?php endif; ?>
						<?php else: ?>
	    		...
	    		<div id="wcvendor_profile_act_btns" class="wcvendor_profile_cell">
				    ...
				    <?php if ( class_exists( 'BuddyPress' ) ) : ?>
						...
						<?php if ( bp_is_active('messages') && !$active_contact_widget ) { ?>
							...
							<a href="<?php echo esc_url($link); ?>" class="wpsm-button medium white<?php echo esc_attr($class); ?>"><?php _e('Contact vendor', 'rehub-theme'); ?></a>
						<?php } ?>
					<?php endif; ?>
					<?php if ( $active_contact_widget ) { ?>
						<span class="wpsm-button rehub_scroll medium white" data-scrollto="#wcv_pro_quick_contact_form"><?php _e( 'Contact vendor', 'rehub-theme' ); ?></span>
					<?php } ?>
	    		</div>	        			
			</div>
		</div>
	...
	<div id="wcvendor_profile_menu">
		<div class="rh-container litesearchstyle">			
			...
			if(isset($_GET['rh_wcv_vendor_cat']) && !empty($_GET['rh_wcv_vendor_cat'])): 
				...
				$placeholder = __('Search in this category...', 'rehub-theme');
			else :
				$search_in_cat = '';
				$placeholder = __('Search in this shop...', 'rehub-theme');
			endif; 
			?>
			...
			<ul class="wcvendor_profile_menu_items">		
				<li class="active"><a href="#vendor-items" aria-controls="vendor-items" role="tab" data-toggle="tab" aria-expanded="true"><?php _e('Items', 'rehub-theme');?></a></li>
				<?php if ( class_exists( 'WCVendors_Pro' ) ) :?>
					...
					<?php if ( $feedback_form_page ) :?>
						...
						<li><a href="<?php echo esc_url($url); ?>"><?php _e('Reviews', 'rehub-theme');?></a></li>	
					<?php endif;?>
				<?php endif;?>
				<?php if( !$active_short_description ) : ?>
				<li><a href="#vendor-about" aria-controls="vendor-about" role="tab" data-toggle="tab" aria-expanded="true" data-scrollto="#vendor-about"><?php _e('About', 'rehub-theme');?></a>
				</li>
				<?php endif; ?>
			</ul>
		</div>
    ...
    <div class="rh-content-wrap clearfix">
	    ...
	    <aside class="rh-mini-sidebar user-profile-div floatleft tabletblockdisplay">
	    	<div class="rh-cartbox widget">
	            <div>
	            	<div class="widget-inner-title rehub-main-font"><?php _e('Shop owner:', 'rehub-theme');?></div>
                    ...
                    <div><i class="far fa-user"></i> <?php _e( 'Registration', 'rehub-theme' );  echo ': ' .date_i18n( get_option( "date_format" ), strtotime( $vendor->user_registered )); ?></div>
	                    <div><i class="far fa-heartbeat"></i><?php _e( 'Product Votes', 'rehub-theme' ); echo ': ' . $count_p_votes; ?></div>
	                    <div><i class="far fa-briefcase"></i><?php _e( 'Total submitted', 'rehub-theme' ); echo ': ' . $totaldeals; ?></div>
					...
					<?php if ( class_exists( 'WCVendors_Pro' ) ) : ?>
						<div class="profile-description">
							<div>
								<span><?php _e( 'Contact', 'rehub-theme' ); ?></span>
								<p>
								<?php if( !$active_address_and_map ) : ?>
	            ...
	            <?php if ( !empty( $vendor->description ) ) : ?>
	                <div class="profile-description">
	                    <div>
	                        <span><?php _e( 'About author', 'rehub-theme' ); ?></span>
                    ...
                    <div class="profile-usermenu mt20">
	                    <ul class="user-menu-tab" role="tablist">
	                        <li class="text-center">
	                            <a href="<?php echo bp_core_get_user_domain( $vendor_id ); ?>"><i class="far fa-folder-open"></i><?php _e( 'Show full profile', 'rehub-theme' ); ?></a>
	                        </li>
	                    </ul>
                    </div>
                <?php endif; ?>
