			...
			<div id="rehub-login-popup-block">
				<?php if (rehub_option('custom_register_link') ==''):?>
					...
					<div class="rehub-register-popup">	 
						<div class="re_title_inmodal"><?php _e('Register New Account', 'rehub-theme'); ?></div>
						...
						<form id="rehub_registration_form_modal" action="<?php echo home_url( '/' ); ?>" method="POST">
							...
							<div class="re-form-group mb20">
								<label><?php _e('Username', 'rehub-theme'); ?></label>
								<input class="re-form-input required" name="rehub_user_login" type="text"/>
							</div>
							<div class="re-form-group mb20">
								<label for="rehub_user_email"><?php _e('Email', 'rehub-theme'); ?></label>
								<input class="re-form-input required" name="rehub_user_email" id="rehub_user_email" type="email"/>
							</div>
							<div class="re-form-group mb20">
								<label for="rehub_user_signonpassword"><?php _e('Password', 'rehub-theme'); ?><span class="alignright font90"><?php _e('Minimum 6 symbols', 'rehub-theme');  ?></span></label>
								<input class="re-form-input required" name="rehub_user_signonpassword" id="rehub_user_signonpassword" type="password"/>
							</div>
							<div class="re-form-group mb20">
								<label for="rehub_user_confirmpassword"><?php _e('Confirm password', 'rehub-theme'); ?></label>
							...
							<div class="re-form-group mb20">
								...
								<button class="wpsm-button rehub_main_btn" type="submit"><?php _e('Sign up', 'rehub-theme'); ?></button>
							</div>
							<?php wp_nonce_field( 'ajax-login-nonce', 'register-security' ); ?>
						</form>
						<div class="rehub-errors"></div>
						<div class="rehub-login-popup-footer"><?php _e('Already have an account?', 'rehub-theme'); ?> <span class="act-rehub-login-popup color_link" data-type="login"><?php _e('Login', 'rehub-theme'); ?></span></div>
					</div>
					</div>
				<?php endif;?>
				...
				<div id="rehub-login-popup">
			 	<div class="rehub-login-popup">
					<div class="re_title_inmodal"><?php _e('Log In', 'rehub-theme'); ?></div>
					<?php if (rehub_option('custom_msg_popup') !='') {
						echo '<div class="mb15 mt15 rh_custom_msg_popup">';
						echo do_shortcode(rehub_option('custom_msg_popup'));
						echo '</div>';
					...
					<?php if(class_exists('\CashbackTracker\application\Plugin')):?>
						<div class="blue_type nonefloat_box wpsm_box rhhidden mb15" id="rh-ca-login">
							...
							<a href="#" target="_blank" id="rh-ca-login-a"><?php _e('go to site', 'rehub-theme');?></a> <?php _e('without cashback', 'rehub-theme');?>
						</div>
					<?php endif;?>					
					<form id="rehub_login_form_modal" action="<?php echo home_url( '/' ); ?>" method="post">
						...
						<div class="re-form-group mb20">
							<label for="rehub_user_pass"><?php _e('Password', 'rehub-theme')?></label>
							<input class="re-form-input required" name="rehub_user_pass" id="rehub_user_pass" type="password"/>
							<?php if(class_exists('Woocommerce')) :?>
								<a href="<?php echo wc_lostpassword_url(); ?>" class="alignright"><?php _e('Lost Password?', 'rehub-theme'); ?></a>
							<?php else: ?>
								<span class="act-rehub-login-popup color_link alignright" data-type="resetpass"><?php _e('Lost Password?', 'rehub-theme');  ?></span>
							<?php endif;?>							
						</div>
						<div class="re-form-group mb20">
							<label for="rehub_remember"><input name="rehub_remember" id="rehub_remember" type="checkbox" value="forever" />
							<?php _e('Remember me', 'rehub-theme'); ?></label>
						</div>						
						<div class="re-form-group mb20">
							<input type="hidden" name="action" value="rehub_login_member_popup_function"/>
							<button class="wpsm-button rehub_main_btn" type="submit"><?php _e('Login', 'rehub-theme'); ?></button>
						</div>
						<?php wp_nonce_field( 'ajax-login-nonce', 'loginsecurity' ); ?>
					</form>
					<div class="rehub-errors"></div>
					<div class="rehub-login-popup-footer"><?php _e('Don\'t have an account?', 'rehub-theme'); ?> 
					<?php if (rehub_option('custom_register_link') !=''):?>
						<span class="act-rehub-login-popup color_link" data-type="url" data-customurl="<?php echo esc_html(rehub_option('custom_register_link'));?>"><?php _e('Sign Up', 'rehub-theme'); ?></span>						
					<?php else:?>
						<span class="act-rehub-login-popup color_link" data-type="register"><?php _e('Sign Up', 'rehub-theme'); ?></span>
					<?php endif;?>
					</div>
				</div>
				</div>
				...
				<div id="rehub-reset-popup">
			 	<div class="rehub-reset-popup">
					<div class="re_title_inmodal"><?php _e('Reset Password', 'rehub-theme'); ?></div>
					<form id="rehub_reset_password_form_modal" action="<?php echo home_url( '/' ); ?>" method="post">
						<div class="re-form-group mb20">
							<label for="rehub_user_or_email"><?php _e('Username or E-mail', 'rehub-theme') ?></label>
							<input class="re-form-input required" name="rehub_user_or_email" id="rehub_user_or_email" type="text"/>
						</div>
						<div class="re-form-group mb20">
							<input type="hidden" name="action" value="rehub_reset_password_popup_function"/>
							<button class="wpsm-button rehub_main_btn" type="submit"><?php _e('Get new password', 'rehub-theme'); ?></button>
						</div>
...
function rehub_lostPassword_retrieve( $user_data ) {
	...
	if(empty($user_data)){
		$errors->add( 'empty_username', __( 'Please enter a username or e-mail address.', 'rehub-theme' ) );
	}elseif(strpos($user_data, '@')){
		$user_data = get_user_by( 'email', trim( $user_data ) );
		if(empty($user_data)){
			$errors->add( 'invalid_email', __( 'There is no user registered with that email address.', 'rehub-theme'  ) );
		}
	}else{
	...
	if(!$user_data){
		$errors->add('invalidcombo', __('Invalid username or e-mail.', 'rehub-theme'));
		return $errors;
	}
	...
	if(!$allow){
		return new WP_Error( 'no_password_reset', __( 'Password reset is not allowed for this user', 'rehub-theme' ) );
	}
	...
	$message = __('Someone requested password reset for the following account:', 'rehub-theme' ) . "\r\n\r\n";
	$message .= network_home_url( '/' ) . "\r\n\r\n";
	$message .= sprintf( __( 'Username: %s', 'rehub-theme' ), $user_login ) . "\r\n\r\n";
	$message .= __('If this was a mistake, just ignore this email and nothing will happen.', 'rehub-theme' ) . "\r\n\r\n";
	$message .= __('To reset your password, visit the following address:', 'rehub-theme' ) . "\r\n\r\n";
	...
	$title   = sprintf( __( '[%s] Password Reset', 'rehub-theme' ), $from_name );
	...
	if ( $message) {
		...
		if(!$sendmessage){
			$errors->add( 'noemail', __( 'The e-mail could not be sent.<br />Possible reason: your host may have disabled the mail function or RH Framework is not installed.', 'rehub-theme' ) );
			return $errors;
			wp_die();
		}
	}
