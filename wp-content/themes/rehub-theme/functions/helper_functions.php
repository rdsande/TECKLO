...
function dimox_breadcrumbs() {
  ...
  /* === OPTIONS === */
  $text['home'] = __('Home', 'rehub-theme');
  $text['category'] = __('Archive category "%s"', 'rehub-theme');
  $text['search'] = __('Search results for "%s"', 'rehub-theme');
  $text['tag'] = __('Posts with tag "%s"', 'rehub-theme');
  $text['author'] = __('Author archive "%s"', 'rehub-theme');
  $text['404'] = __('Error 404', 'rehub-theme');
...
function rehub_contents_shortcode( $content ){
    $args = array();
    $args['to_menu']  = __('back to menu', 'rehub-theme').' ↑';
...
function rh_get_crosstaxonomy( $parent, $ids, $showtax ) {
    ...
    ("
        ...
        WHERE
            t1.taxonomy = '$parent' AND p1.post_status = 'publish' AND terms1.term_id IN (".$ids.") AND
            t2.taxonomy = '$showtax' AND p2.post_status = 'publish'
            AND p1.ID = p2.ID
        ORDER by tag_name
    ");
    $count = 0;
...
function rh_get_product_position( $id, $taxonomy = 'product_cat' ){
  ...
  $sql_query = "SELECT SQL_CALC_FOUND_ROWS {$db_prefix}posts.ID FROM {$db_prefix}posts LEFT JOIN {$db_prefix}term_relationships ON ({$db_prefix}posts.ID = {$db_prefix}term_relationships.object_id) INNER JOIN {$db_prefix}postmeta ON ({$db_prefix}posts.ID = {$db_prefix}postmeta.post_id ) WHERE 1=1 AND ({$db_prefix}term_relationships.term_taxonomy_id IN ({$str_terms})) AND ({$db_prefix}postmeta.meta_key = 'rehub_review_overall_score') AND {$db_prefix}posts.post_type = 'product' AND ({$db_prefix}posts.post_status = 'publish') GROUP BY {$db_prefix}posts.ID ORDER BY {$db_prefix}postmeta.meta_value+0 DESC";
