...
function rehub_contactmethods( $contactmethods ) {
    ...
    $contactmethods['twitter']   = __('Url of Twitter page', 'rehub-theme');
    $contactmethods['facebook']  = __('Url of Facebook page', 'rehub-theme');
    $contactmethods['google']    = __('Url of Google Plus page', 'rehub-theme');
    $contactmethods['tumblr']    = __('Url of Tumblr page', 'rehub-theme');
    $contactmethods['instagram'] = __('Url of Instagram page', 'rehub-theme');
    $contactmethods['vkontakte'] = __('Url of Vk.com page', 'rehub-theme');
    $contactmethods['youtube'] = __('Url of Youtube page', 'rehub-theme');
            ...
            <div class="author_detail_box clearfix"><?php echo get_avatar( get_the_author_meta('email'), '69' ); ?>
                <div class="clearfix">
                    <?php if ( function_exists('bp_core_get_user_domain') ) : ?>
                        <a href="<?php echo bp_core_get_user_domain( $author_ID ); ?>" class="see_full_profile_btn"><?php _e( 'Show full profile', 'rehub-theme' ); ?></a>
                    <?php endif; ?>                
                    <h4>
...
if ( function_exists( 'mycred' ) ) {
    ...
    function mycred_register_overall_post_likes_hook( $installed ) {
        $installed['overallpostlikes'] = array(
            'title' => __( 'Hot Meter & Thumbs Likes', 'rehub-theme' ),
            'description' => __( 'Awards %_plural% to Author for post likes via the Hot Meter.', 'rehub-theme' ),
            'callback' => array( 'myCRED_Hook_Overall_Post_Likes' )
        );
        $installed['overallpostwishes'] = array(
            'title' => __( 'Wishlist', 'rehub-theme' ),
            'description' => __( 'Awards %_plural% to Author for adding his post to wishlist', 'rehub-theme' ),
    ...
    function add_overall_post_likes_references( $references ) {      
        $references['ref_overall_post_likes'] = __( 'Hot Meter & Thumbs Likes', 'rehub-theme' );
        $references['ref_overall_post_wishes'] = __( 'Wishlist', 'rehub-theme' );        
        return $references;
    }
    ...
    function mycred_load_overall_post_likes_hook() {
        ...
        class myCRED_Hook_Overall_Post_Likes extends myCRED_Hook {
            ...
            $prefs = $this->prefs;
            ?>
                <label class="subheader" for="<?php echo ''.$this->field_id( array( 'added' => 'creds' ) ); ?>"><?php _e( 'Author Content is liked', 'rehub-theme' ); ?></label>
                <ol>
                    <li>
                    ...
                    <li>
                        <label for="<?php echo ''.$this->field_id( array( 'added' => 'limit' ) ); ?>"><?php _e( 'Limit', 'rehub-theme' ); ?></label>
                        <?php echo ''.$this->hook_limit_setting( $this->field_name( array( 'added' => 'limit' ) ), $this->field_id( array( 'added' => 'limit' ) ), $prefs['added']['limit'] ); ?>
                    </li>
                </ol>
                <label class="subheader" for="<?php echo ''.$this->field_id( array( 'added' => 'log' ) ); ?>"><?php _e( 'Log Template', 'rehub-theme' ); ?></label>
                <ol>
                    <li>
                ...
                <label class="subheader" for="<?php echo ''.$this->field_id( array( 'removed' => 'creds' ) ); ?>"><?php _e( 'Author Content is disliked', 'rehub-theme' ); ?></label>
                <ol>
                    <li>
                ...
                <label class="subheader" for="<?php echo ''.$this->field_id( array( 'removed' => 'log' ) ); ?>"><?php _e( 'Log Template', 'rehub-theme' ); ?></label>
                <ol>
                    <li>
        ...
        class myCRED_Hook_Overall_Post_Wishes extends myCRED_Hook {
            ...
            $prefs = $this->prefs;
            ?>
                <label class="subheader" for="<?php echo ''.$this->field_id( array( 'added' => 'creds' ) ); ?>"><?php _e( 'Author Content is added to wishlist', 'rehub-theme' ); ?></label>
                <ol>
                    <li>
                        <div class="h2"><input type="text" name="<?php echo ''.$this->field_name( array( 'added' => 'creds' ) ); ?>" id="<?php echo ''.$this->field_id( array( 'added' => 'creds' ) ); ?>" value="<?php echo ''.$this->core->number( $prefs['added']['creds'] ); ?>" size="8" /></div>
                    </li>
