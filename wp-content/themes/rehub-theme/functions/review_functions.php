...
function rehub_get_user_rate($schema='admin', $type = 'post', $customid = ''){
	...
	if( !empty($disable_rate) ){
		$no_rate_text = __( 'No Ratings Yet!', 'rehub-theme' );
		$rate_active = false ;
	}
	else{
		$no_rate_text = __( 'Be the first one!' , 'rehub-theme' );
	...
	if ( $user_ID ) {
		...
		if( !empty($user_rated) && is_array($user_rated) && array_key_exists($postid , $user_rated) ){
			...
			return $output = '<div class="rh-star-ajax"><span class="title_star_ajax"><strong>'.__( "User Rating:" , "rehub-theme" ) .' </strong> <span class="userrating-score">'.$total_users_score.'</span> <small>(<span class="userrating-count">'.$count.'</span> '.__( "votes" , "rehub-theme" ) .')</small> </span><div data-rate="'. round($total) .'" data-ratetype="'.$type.'" class="rate-post-'.$postid.' user-rate rated-done"><span class="post-norsp-rate '.$image_style.'-rate-ajax-type">'.$stars.'</span></div><div class="userrating-clear"></div></div>';
		}
	}else{
		$ip = rehub_get_ip();
		$ip = str_replace('.', '', $ip);
		...
		if( !empty($get_t_id) ){
			if (array_key_exists($ip, $get_t_id)){			
				return $output = '<div class="rh-star-ajax"><span class="title_star_ajax"><strong>'.__( "User Rating:" , "rehub-theme" ) .' </strong> <span class="userrating-score">'.$total_users_score.'</span> <small>(<span class="userrating-count">'.$count.'</span> '.__( "votes" , "rehub-theme" ) .')</small> </span><div data-rate="'. round($total) .'" class="rate-post-'.$postid.' user-rate rated-done"><span class="post-norsp-rate '.$image_style.'-rate-ajax-type">'.$stars.'</span></div><div class="userrating-clear"></div></div>';
			}
		}
	...
	if( $total == 0 && $count == 0)
		...
		return $output = '<div class="rh-star-ajax"><span class="title_star_ajax"><strong>'.__( "User Rating:" , "rehub-theme" ) .' </strong> <span class="userrating-score">'.$total_users_score.'</span> <small>(<span class="userrating-count">'.$count.'</span> '.__( "votes" , "rehub-theme" ) .')</small> </span><div data-rate="'. $total .'" data-id="'.$postid.'" data-ratetype="'.$type.'" class="rate-post-'.$postid.' user-rate'.$rate_active.'"><span class="post-norsp-rate '.$image_style.'-rate-ajax-type">'.$stars.'</span></div><div class="userrating-clear"></div></div>';
}
}
...
function rehub_get_user_rate_criterias (){
	...
	if ($postAverage !='0' && $postAverage !='' ){
		...
		return $output = '<div class="star"><span class="title_stars"><strong>'.__( "User Rating:" , "rehub-theme" ) .' </strong> <span class="userrating-score">'.$postAverage.'/10</span> <small>(<span class="userrating-count">'.$userrevcount['criteria'][0]['count'].'</span> '.__( "votes" , "rehub-theme" ) .')</small></span><div class="user-rate"><span class="stars-rate"><span style="width: '.$total.'%;"></span></span></div></div>';
	}
	else {
...
function rehub_get_user_results( $size = 'small', $words = 'no' ){
	...
	if ((rehub_option('type_user_review') == 'full_review') && ($postAverage !='0' && $postAverage !='' )){
		$total = $postAverage*10;
		?>
		<?php if ($words == 'yes') :?><strong><?php _e('User rating', 'rehub-theme'); ?>: </strong><?php endif ;?><div class="star-<?php echo ''.$size ?>"><span class="stars-rate"><span style="width: <?php echo ''.$total ?>%;"></span></span></div>
		<?php
	}
		...
		<?php if ($overal_score !='0') :?>
			<div class="review-top">								
				<div class="overall-score">
					...
					<span class="overall-text"><?php _e('Total Score', 'rehub-theme'); ?></span>
					...
					<div class="overall-user-votes"><span><?php $user_rates = get_post_meta(get_the_ID(), 'post_user_raitings', true); echo ''.$user_rates['criteria'][0]['count'] ;?></span> <?php _e('reviews', 'rehub-theme'); ?></div>
					<?php endif;?>				
				</div>				
		...
		<?php if ((rehub_option('type_user_review') == 'full_review') && ($postAverage !='0' && $postAverage !='' )) :?>
			<div class="rate_bar_wrap_two_reviews">
				...
				<div class="review-criteria">
					<div class="l_criteria"><span class="score_val r_score_<?php echo round(rehub_get_overall_score_editor()); ?>"><?php echo round(rehub_get_overall_score_editor(), 1); ?></span><span class="score_tit"><?php _e('Expert Score', 'rehub-theme'); ?></span></div>
				...
				<div class="review-criteria user-review-criteria">
					<div class="l_criteria"><span class="score_val r_score_<?php echo round($postAverage); ?>"><?php echo round($postAverage, 1) ?></span><span class="score_tit"><?php _e('User\'s score', 'rehub-theme'); ?></span></div>
					<div class="r_criteria">
...
function getCommentLike_re( $comment_text  ) {
	...
	$output .= '<span class="us-rev-vote-up'.$already.'" data-post_id="'.$post_id.'" data-informer="'.$comm_plus_count.'"><i class="far fa-thumbs-up"></i> <span class="comm_help_title">'.__('Helpful', 'rehub-theme').'</span>(<span class="help_up_count" id="commhelpplus'.$post_id.'">'.$comm_plus_count.'</span>)</span>'; 
	$output .= '<span class="us-rev-vote-down'.$already.'" data-post_id="'.$post_id.'" data-informer="'.$comm_minus_count.'"><i class="far fa-thumbs-down"></i> <span class="comm_help_title">'.__('Unhelpful', 'rehub-theme').'</span>(<span class="help_up_count" id="commhelpminus'.$post_id.'">'.$comm_minus_count.'</span>)</span>'; 
	$output .= '<span class="already_commhelp">'.__('You have already voted this', 'rehub-theme').'</span></div>';
