	...
	<?php global $woocommerce; if($woocommerce && $review_woo_link) :?>
		...
		<?php $products = new WP_Query( $args ); if ( $products->have_posts() ) : ?>
    		<?php while ( $products->have_posts() ) : $products->the_post(); global $product?>
	            ...
	            <?php if(rehub_option('rehub_btn_text') !='') :?><?php $btn_txt = rehub_option('rehub_btn_text') ; ?><?php else :?><?php $btn_txt = __('Buy It Now', 'rehub-theme') ;?><?php endif ;?>
    			...
    			<div class="rehub_woo_review woocommerce">
    				<?php if (!empty ($attributes) || !empty ($gallery_images) || !empty ($woo_aff_links_inreview)) :?>
    					<ul class="rehub_woo_tabs_menu">
				            <li><?php _e('Product', 'rehub-theme') ?></li>
				            <?php if (!empty ($attributes)) :?><li><?php _e('Specification', 'rehub-theme') ?></li><?php endif ;?>
				            <?php if (!empty ($gallery_images)) :?><li><?php _e('Photos', 'rehub-theme') ?></li><?php endif ;?>
				            <?php if (!empty ($post_image_videos)) :?><li><?php _e('Videos', 'rehub-theme') ?></li><?php endif ;?>				            
				            <?php if (!empty ($woo_aff_links_inreview)) :?><li class='woo_deals_tab'><?php _e('Deals', 'rehub-theme') ?></li><?php endif ;?>
						</ul>
						<?php endif ;?>
							            ...
							            <div class="priced_block clearfix">
							                <div>
							                	<?php if ($product->get_type() =='external' && $product->add_to_cart_url() ==''  && !empty ($woo_aff_links_inreview)) :?>
							                		<a class='btn_offer_block choose_offer_woo' href="#"><?php _e('Check Deals', 'rehub-theme') ;?></a>
							                	<?php else :?>
...
function rehub_generate_offerbtn($args) {
	...
	?>
		    		...
		    		<?php if($showme !='price') : ?>
			    		<div class="post_offer_anons">
		                	...
		                	<span class="coupon_btn re_track_btn btn_offer_block rehub_offer_coupon masked_coupon <?php if(!empty($offer_coupon_date)) {echo ''.$coupon_style ;} ?>" data-clipboard-text="<?php echo esc_html ($offer_coupon) ?>" data-codeid="<?php echo (int)$post->ID?>" data-dest="<?php echo esc_url($offer_url) ?>">
		                		...
		                		<?php else :?>
		                			<?php _e('Reveal coupon', 'rehub-theme') ?>
		                		<?php endif ;?>
		                	</span>
		            	</div>
	            	<?php endif;?>
        ...
        <?php endif ;?>
    	<?php if ($showme !='price') :?>
    		<?php if ( $btn_more =='both' || (!$offer_url_exist && $btn_more =='yes')) :?>
				...
				<?php elseif (rehub_option('rehub_readmore_text') !=''): ?>
			  	...
			  	<?php else: ?>
					<a href="<?php the_permalink();?>" class="btn_more"><?php _e('READ MORE  +', 'rehub-theme') ;?></a>
			  	<?php endif ?>
		  	<?php endif ;?>
    	<?php endif ;?>
...
function rehub_quick_offer($id=''){
	...
	$disclaimer = get_post_meta($post->ID, 'rehub_offer_disclaimer', true);
...
function set_content_end($content) {
	...
	$output = '';
	ob_start();
	wp_link_pages(array( 'before' => '<div class="page-link"><span class="page-link-title">' . __( 'Pages:', 'rehub-theme' ).'</span>', 'after' => '</div>', 'pagelink' => '<span>%</span>' ));
	$output .= ob_get_clean();
	...
	if (!empty($reviewCriteria[$form_id])){
	    ...
	    <div id="user_reviews_in_frontend" class="rate_bar_wrap">
	    	<div class="wpfepp-form-field-container">
	    		<label><?php _e('Review heading', 'rehub-theme');?></label>
	        	<input type="text" name="review_heading" value="<?php echo ''.$review_heading; ?>" />
	        </div>
	        <div class="wpfepp-form-field-container">
				<label><?php _e('Review summary', 'rehub-theme');?></label>
	        	<textarea name="review_summary"><?php echo ''.$review_summary; ?></textarea>
	        </div>
	        <div class="wpfepp-form-field-container">
				<label><?php _e('PROS. Add each from separate line', 'rehub-theme');?></label>
	        	<textarea name="review_post_pros_text"><?php echo ''.$review_proses; ?></textarea>
	        </div>
	        <div class="wpfepp-form-field-container">
				<label><?php _e('CONS. Add each from separate line', 'rehub-theme');?></label>
...
if (!function_exists('rehub_change_post_query')){
	function rehub_change_post_query($q){
		if (rehub_option('rehub_post_exclude_expired') == '1') {
		    ...
		    if (is_post_type_archive('post') || is_category() || is_home() || is_feed()) {
