...
if (rehub_option('woo_cart_place') =='1' || rehub_option('woo_cart_place') =='2' || (rehub_option('rehub_header_style') =='header_seven' && rehub_option('header_seven_cart') != '')){
	...
	function woocommerce_header_add_to_cart_fragment( $fragments ) {
		...
		<?php if (rehub_option('woo_cart_place') =='1'):?>
			<a class="cart-contents cart_count_<?php echo ''.$woocommerce->cart->cart_contents_count; ?>" href="<?php echo wc_get_cart_url(); ?>"><i class="far fa-shopping-cart"></i> <?php _e( 'Cart', 'rehub-theme' ); ?> (<?php echo ''.$woocommerce->cart->cart_contents_count; ?>) - <?php echo ''.$woocommerce->cart->get_cart_total(); ?></a>		
...
//////////////////////////////////////////////////////////////////
// Custom Editor Review and User ratings
//////////////////////////////////////////////////////////////////
...
if(!function_exists('rh_woo_get_editor_rating')){
	function rh_woo_get_editor_rating(){
		...
		if($editor_rating > 0){
			$html = '<div class="rh_woo_star" title="'.sprintf( __( 'Rated %s out of', 'rehub-theme' ), esc_html( (float)$editor_rating )).' 10">';
			$editor_rating = $editor_rating/2;
			for ($i = 1; $i <= 5; $i++){
		    	if ($i <= $editor_rating){
		    		$active = ' active';
		    	}else{
...
function rh_woo_rating_icons_wrapper($html, $rating, $count){
	if ( 0 < $rating ) {
		$html  = '<div class="rh_woo_star" title="'.sprintf( __( 'Rated %s out of', 'rehub-theme' ), esc_html( (float)$rating )).' 5">';
...
if ( !function_exists('rh_show_vendor_info_single') ) {
	function rh_show_vendor_info_single($wrapperclass='') {
		...
		if( class_exists( 'WeDevs_Dokan' ) ) {
			...
			$sold_by_label = apply_filters( 'dokan_sold_by_label', __( 'Sold by', 'rehub-theme' ) );
			...
			$sold_by_label = (!empty($wcmp_option['sold_by_text'])) ? $wcmp_option['sold_by_text'] : __( 'Sold by', 'rehub-theme' );
		}
		...
		if($is_vendor){
			...
			if(class_exists( 'WeDevs_Dokan' ) && dokan_get_option( 'contact_seller', 'dokan_general', 'on' ) == 'on'){
				...
				echo ' <a href="'.$store_url.'#dokan-form-contact-seller" class="vendor_store_owner_contactlink'.$class.'"><i class="far fa-envelope" aria-hidden="true"></i> <span>'. __('Ask owner', 'rehub-theme') .'</span></a>';									
				echo '</span>';					
			}
			elseif(is_active_widget( '', '', 'dc-vendor-quick-info')){
				...
				$class = ( !is_user_logged_in() && rehub_option( 'userlogin_enable' ) == '1' ) ? ' act-rehub-login-popup' : '';						
				echo ' <a href="'.$store_url.'#wcmp-vendor-contact-widget-top" class="vendor_store_owner_contactlink'.$class.'"><i class="far fa-envelope" aria-hidden="true"></i> <span>'. __('Ask owner', 'rehub-theme') .'</span></a>';									
				echo '</span>';	
			}	
			elseif(class_exists( 'BuddyPress' ) ) {
				...
				if ( bp_is_active( 'messages' )){
					...
					$class = (!is_user_logged_in() && rehub_option('userlogin_enable') == '1') ? ' act-rehub-login-popup' : '';
						echo ' <a href="'.$link.'" class="vendor_store_owner_contactlink'.$class.'"><i class="far fa-envelope" aria-hidden="true"></i> <span>'. __('Ask owner', 'rehub-theme') .'</span></a>';			
				}
				echo '</span>';		
			}
...
if (rehub_option('wooregister_xprofile') == 1){
	...
	function rh_add_xprofile_to_dokan( $user_id ) {
		if ( class_exists( 'BuddyPress' ) ) {
			...
			<h2><?php _e( 'Extended Profile', 'rehub-theme' ); ?></h2>
		...
		if ( class_exists( 'BuddyPress' ) ) {
			if (!empty($_POST['signup_profile_field_ids'])){
				...
				if(!empty($user_error_req_fields)){
		        	$validation_errors->add( 'billing_first_name_error', __( ' Next fields are required: ', 'rehub-theme' ).implode(', ',$user_error_req_fields) );									
				}			
			}
		}	 
		...
		function wcv_add_custom_submit_links($items){
			if (rehub_option('url_for_add_product') && !empty($items['submit_link'])){
				...
				$items['submit_link'] = array(
					...
					'label'  => __('Add New Product', 'rehub-theme'),
					'target' => '_top',
				);
			}
			if (rehub_option('url_for_edit_product') && !empty($items['edit_link'])){
				...
				$items['edit_link']   = array(
					...
					'label'  => __('Edit Products', 'rehub-theme'),
					'target' => '_top',
				);				
			}
			return $items;			
...
if( class_exists( 'WeDevs_Dokan' ) ) {
	...
	function custom_dokan_admin_settings( $settings_fields ){
		$settings_fields['dokan_general']['store_map']['desc']  = __( 'Enable showing link to Store location map on store', 'rehub-theme' );
	...
	function custom_dokan_toc_url( $tabs ){
		$tabs['terms_and_conditions'] = array(
			'title' => apply_filters( 'dokan_about_store_title', __( 'Terms and Conditions', 'rehub-theme' ) ),
			'url'   => '#vendor-about'
		);
		return $tabs;
	}
	...
	function dokan_loop_sold_by() {
		...
		$sold_by = dokan_is_user_seller( $vendor_id )
			...
			: get_bloginfo( 'name' );
		?>
		<small class="wcvendors_sold_by_in_loop"><span><?php echo apply_filters( 'dokan_sold_by_label', __( 'Sold by', 'rehub-theme' ) ); ?></span> <?php echo ''.$sold_by; ?></small><br />
		<?php
	}
	...
	function wcmprh_loop_sold_by(){
		...
		if ('Enable' === get_wcmp_vendor_settings('sold_by_catalog', 'general') && apply_filters('wcmp_sold_by_text_after_products_shop_page', true, $post->ID)){
			...
			if($is_vendor && empty($multivendor_product['more_product_array'])){
				...
				$sold_by = apply_filters('rh_sold_by_multivendor', __('Multiple vendor', 'rehub-theme'), $multivendor_product['more_product_array']);
			}else{
				$sold_by = apply_filters('rh_sold_by_site', get_bloginfo('name'));
			}
			$sold_by_text = apply_filters('wcmp_sold_by_text', __('Sold By', 'rehub-theme'));
			...
			if(current_user_can('edit_products')) {
				...
				if(!empty($userlogin_submit_page)) {
					$vendor_nav['vendor-products']['submenu']['add-new-product'] = array(
						'label' => __('Add Product', 'rehub-theme'),
...
if(defined( 'WCFMmp_TOKEN' )){
	...
	function rh_wcfm_sidebar_args(){
		return array(
			...
			'name'          => __( 'Vendor store page sidebar', 'rehub-theme' ),
	...
	function rh_wcfm_dash_color_settings($args){
		...
		$args['wcfm_field_button_text_color']['default'] = '#ffffff';
	...
	function rh_wcfm_store_color_settings($args){
		...
		$args['wcfmmp_button_active_bg_color']['default'] = $btncolor;
