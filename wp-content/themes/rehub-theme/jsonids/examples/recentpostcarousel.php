...
jQuery(document).ready(function() {
	...
	jQuery('#submit').click(function(){
				...
				var shortcode = '[post_carousel_mod';
		
				if(Postslidernumber) {
					shortcode += ' show="'+Postslidernumber+'"';
				}
				if(Postslidercat) {
					shortcode += ' cat="'+Postslidercat+'"';
				}
...
<form action="/" method="get" id="form" name="form" accept-charset="utf-8">
    <p>
		<label for="Postslidernumber"><?php _e('Number of posts to show', 'rehub-theme') ;?></label>
		<input id="Postslidernumber" name="Postslidernumber" type="text" />
		<small><?php _e('Minimum 4', 'rehub-theme') ;?></small>
	</p>
	<p>
		<label for="Postslidercat"><?php _e('Category ID (optional) :', 'rehub-theme') ;?></label>
