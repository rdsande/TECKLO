...
<?php if(rehub_option('rehub_widget_comments') && comments_open()) : ?><?php echo wp_specialchars_decode( stripslashes(rehub_kses(rehub_option('rehub_widget_comments')))); ?><div style="margin-bottom:15px; clear:both"></div><?php endif; ?>
<?php if(rehub_option('rehub_disable_comments') != '1') :?>
    <div class="post-comments">
        <?php
            if ( comments_open() ) :
            echo "<div class='title_comments'>";
            $title_nocomments = (rehub_option('rehub_commenttitle_text') != '') ? rehub_option('rehub_commenttitle_text') : __('We will be happy to hear your thoughts','rehub-theme');
            comments_number($title_nocomments, __('1 Comment','rehub-theme'), '% ' . __('Comments','rehub-theme') );
            echo "</div>";
            endif;
        ?>
    <?php if ((rehub_option('type_user_review') == 'full_review' || rehub_option('type_user_review') == 'user') && vp_metabox('rehub_post.rehub_framework_post_type') == 'review' && get_comments_number() > 1) :?>
        <div id="rehub-comments-tabs" class="mb30" data-postid = "<?php echo get_the_ID();?>">
            <span data-tabID="1" data-posttype="post" class="active"><?php esc_html_e('Show all', 'rehub-theme'); ?></span>
            <span data-tabID="2" data-posttype="post"><?php esc_html_e('Most Helpful', 'rehub-theme'); ?></span>
            <span data-tabID="3" data-posttype="post"><?php esc_html_e('Highest Rating', 'rehub-theme'); ?></span>
            <span data-tabID="4" data-posttype="post"><?php esc_html_e('Lowest Rating', 'rehub-theme'); ?></span>
    ...
    <ol id="loadcomment-list" class="commentlist">
        ...
        <?php
            ...
            comment_form(array(
                ...
                'title_reply'           => __('Leave a reply', 'rehub-theme'),
                'cancel_reply_link'     => __('Cancel reply', 'rehub-theme'),
