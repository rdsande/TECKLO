...
<p>
<?php printf( __( 'Order #<strong class="order-number">%s</strong> was placed on <strong class="order-date">%s</strong> and is currently <strong class="order-status">%s</strong>.', 'rehub-theme' ), $order->get_order_number(), date_i18n( get_option( 'date_format' ), strtotime( $order->order_date ) ), wc_get_order_status_name( $order->get_status() ) ); ?>
</p>
...
foreach ( $products as $product ) {
	...
	if ( is_string( get_post_status( $product['product_id'] ) ) ) { 
		echo '<a href="' . get_permalink( $product_id ) . '">' . $product['name'] . '</a> '. __( 'from', 'rehub-theme' ) .'&nbsp;'. $shop_name.'</br>';
	} else { 	
	...
	<input type="text" name="wcv-feedback[<?php echo ''.$fn; ?>][rating_title]" style="width:60%" value="<?php echo ''.$rating_title; ?>" placeholder="<?php _e( 'Title', 'rehub-theme'); ?>" />
	<textarea name="wcv-feedback[<?php echo ''.$fn; ?>][comments]" style="width:60%" placeholder="<?php _e( 'Comments: (e.g. delivery experience, item as described, quality of customer service)', 'rehub-theme'); ?>"><?php echo strip_tags($comments); ?></textarea>
	<input type="hidden" name="wcv-feedback[<?php echo ''.$fn; ?>][vendor_id]" value="<?php echo ''.$vendor_id; ?>">
	<input type="hidden" name="wcv-feedback[<?php echo ''.$fn; ?>][product_id]" value="<?php echo (int)$product_id; ?>">
...
<p><input type="submit" value="<?php _e('Submit Feedback', 'rehub-theme'); ?>"></p>
