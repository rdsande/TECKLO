...
$searchstr = str_replace('"', '', $searchstr);
$querystr = $wpdb->prepare( "select * from {$wpdb->prefix}wcmp_products_map where replace(replace(product_title, '\'',''), '\"','') = '%s'", $searchstr );
...
if( !empty( $results_str ) ) {
	...
	if(count($results) > 1){
		$button_text = apply_filters('wcmp_more_vendors', __('More offers','rehub-theme'));
