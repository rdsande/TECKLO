...
<aside class="rh-mini-sidebar user-profile-div tabletblockdisplay <?php echo ''.$sideclass;?>">

  <?php do_action( 'wcfmmp_store_before_sidabar', $store_user->get_id() ); ?>
  
  <?php if( !is_active_sidebar( 'sidebar-wcfmmp-store' ) ) { ?>
  	
  		<?php the_widget( 'WCFMmp_Store_Product_Search', array( 'title' => __( 'Search', 'rehub-theme' ) ), $widget_args ); ?>
  	
  		<?php the_widget( 'WCFMmp_Store_Category', array( 'title' => __( 'Categories', 'rehub-theme' ) ), $widget_args ); ?>
		
		<?php the_widget( 'WCFMmp_Store_Location', array( 'title' => __( 'Store Location', 'rehub-theme' ) ), $widget_args ); ?>
