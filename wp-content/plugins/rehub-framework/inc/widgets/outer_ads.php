...
class rehub_outer_mediad extends WP_Widget {
    ...
    function __construct() {
		$widget_ops = array( 'classname' => 'outer_widget', 'description' => __('Outcontent side ads widget.', 'rehub-framework') );
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'rehub_outer_mediad' );
        parent::__construct( 'rehub_outer_mediad', __('ReHub: Side Out Ads widget', 'rehub-framework'), $widget_ops, $control_ops);
    }
...
function widget( $args, $instance ) {
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo ''.$this->get_field_id( 'width' ); ?>"><?php _e('Width of ads (without px):', 'rehub-framework'); ?></label>
		...
		<p>
			<label for="<?php echo ''.$this->get_field_id( 'side' ); ?>"><?php _e('Right Side ?', 'rehub-framework'); ?></label>
		...
		<p>
			<label for="<?php echo ''.$this->get_field_id( 'fixed' ); ?>"><?php _e('Disable fixed position?', 'rehub-framework'); ?></label>
		...
		<p>
			<label for="<?php echo ''.$this->get_field_id( 'margin' ); ?>"><?php _e('Margin from top of page (without px):', 'rehub-framework'); ?></label>
		...
		<p><em><?php _e('Note, if you disable fixed position, margin will be calculated from top of content area', 'rehub-framework');?></em></p>				
		<p>
			<label for="<?php echo ''.$this->get_field_id( 'text_code' ); ?>"><?php _e('Text or Html code :', 'rehub-framework'); ?></label>
