...
class rehub_top_offers_widget extends WP_Widget {
    ...
    function __construct() {
		$widget_ops = array( 'classname' => 'top_offers', 'description' => __('Widget displays top offers. Use only in sidebar!', 'rehub-framework') );
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'rehub_top_offers' );
        parent::__construct('rehub_top_offers', __('ReHub: Top Offers/Products', 'rehub-framework'), $widget_ops, $control_ops );
    }
...
function widget( $args, $instance ) {
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => __('Top offers', 'rehub-framework'), 'number' => 5, 'tag' => '', 'post_type' => 'post', 'order' => '', 'tags' =>'', 'random' =>'', 'orderby'=> 'DESC', 'notexpired'=> '', 'comparebtn'=> '');
		...
		<p>
		<label for="<?php echo ''.$this->get_field_id('post_type'); ?>"><?php _e('Widget is based on:', 'rehub-framework');?></label> 
		<select id="<?php echo ''.$this->get_field_id('post_type'); ?>" name="<?php echo ''.$this->get_field_name('post_type'); ?>" style="width:100%;">
			<option value="post" <?php if ( 'post' == $instance['post_type'] ) : echo 'selected="selected"'; endif; ?>><?php _e('Posts', 'rehub-framework');?></option>
			<option value="woo" <?php if ( 'woo' == $instance['post_type'] ) : echo 'selected="selected"'; endif; ?>><?php _e('Woocommerce', 'rehub-framework');?></option>			
		</select>
		</p>

		<p><em><?php _e('If you select Widget base on posts or woocommerce, enter tag slug in field below. Also, you can set name of meta key for ordering or show random products', 'rehub-framework');?></em></p>

		<p>
			<label for="<?php echo ''.$this->get_field_id( 'tags' ); ?>"><?php _e('Enter tag slug:', 'rehub-framework'); ?></label>
		...
		<p>
			<label for="<?php echo ''.$this->get_field_id( 'order' ); ?>"><?php _e('Meta key name for ordering:', 'rehub-framework'); ?></label>
		...
		<p>
		<label for="<?php echo ''.$this->get_field_id('orderby'); ?>"><?php _e('Order for field above:', 'rehub-framework');?></label> 
		<select id="<?php echo ''.$this->get_field_id('orderby'); ?>" name="<?php echo ''.$this->get_field_name('orderby'); ?>" style="width:100%;">
			<option value="DESC" <?php if ( 'DESC' == $instance['orderby'] ) : echo 'selected="selected"'; endif; ?>><?php _e('DESC', 'rehub-framework');?></option>		
			<option value="ASC" <?php if ( 'ASC' == $instance['orderby'] ) : echo 'selected="selected"'; endif; ?>><?php _e('ASC', 'rehub-framework');?></option>		
		</select>
		</p>
		<p>
			<label for="<?php echo ''.$this->get_field_id( 'comparebtn' ); ?>"><?php _e('Show compare button instead offer button?', 'rehub-framework'); ?></label>
		...
		<p>
			<label for="<?php echo ''.$this->get_field_id( 'random' ); ?>"><?php _e('Show random?', 'rehub-framework'); ?></label>
		...
		<p>
			<label for="<?php echo ''.$this->get_field_id( 'notexpired' ); ?>"><?php _e('Exclude expired?', 'rehub-framework'); ?></label>
