...
class rehub_better_woocat_widget extends WP_Widget {
    ...
    function __construct() {
        $widget_ops = array( 'classname' => 'better_woocat', 'description' => __('Better categories (woocommerce). Use only in sidebar!', 'rehub-framework') );
        $control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'rehub_better_woocat' );
        parent::__construct('rehub_better_woocat', __('ReHub: Better woocommerce categories', 'rehub-framework'), $widget_ops, $control_ops);
    }
...
function widget( $args, $instance ) {
	...
	$showAllLabel = (!empty($instance['showAllLabel'])) ? esc_html($instance['showAllLabel']) : __( 'Show All Categories', 'rehub-framework' );
	$browseAllLabel = (!empty($instance['browseAllLabel'])) ? esc_html($instance['browseAllLabel']) : __( 'Browse Categories', 'rehub-framework' );
	$el_class = '';		
	...
	$list_args['show_option_none']           = __('No product categories exist.', 'rehub-framework' );
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo ''.$this->get_field_id( 'showAllLabel' ); ?>"><?php _e('Label for show all category text:', 'rehub-framework'); ?></label>
		...
		<p>
			<label for="<?php echo ''.$this->get_field_id( 'browseAllLabel' ); ?>"><?php _e('Label for browse categories text:', 'rehub-framework'); ?></label>
		...
		<p>
			<label for="<?php echo ''.$this->get_field_id( 'hideempty' ); ?>"><?php _e('Hide empty categories', 'rehub-framework'); ?></label>
		...
		<p>
			<label for="<?php echo ''.$this->get_field_id( 'woocount' ); ?>"><?php _e('Show count of products', 'rehub-framework'); ?></label>
