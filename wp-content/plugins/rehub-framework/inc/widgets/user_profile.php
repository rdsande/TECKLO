...
class rh_user_profile_widget extends WP_Widget {
    ...
    function __construct() {
		$widget_ops = array( 'classname' => 'user-profile-div', 'description' => __('Widget that displays User Profile on Vendor Store page. Use only in vendor sidebar area!', 'rehub-framework') );
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'rh_user_profile' );
        parent::__construct('rh_user_profile', __('ReHub: User Profile', 'rehub-framework'), $widget_ops, $control_ops );
    }
...
function widget( $args, $instance ) {
		...
		<div class="rh-cartbox widget">
			<div>
				...
				<div class="profile-stats">
					<div><i class="far fa-user"></i> <?php _e( 'Registration', 'rehub-framework' );  echo ': ' .date_i18n( get_option( "date_format" ), strtotime( $vendor->user_registered )); ?></div>
	                <div><i class="far fa-heartbeat"></i><?php _e( 'Product Votes', 'rehub-framework' ); echo ': ' . $count_p_votes; ?></div>
	                <div><i class="far fa-briefcase"></i><?php _e( 'Total submitted', 'rehub-framework' ); echo ': ' . $totaldeals; ?></div>
				</div>
	                ...
	                <div class="profile-usermenu mt20">
	                    <ul class="user-menu-tab" role="tablist">
	                        <li class="text-center">
	                            <a href="<?php echo bp_core_get_user_domain( $vendor_id ); ?>"><i class="far fa-folder-open"></i><?php _e( 'Show full profile', 'rehub-framework' ); ?></a>
	                        </li>
	                    </ul>
	                </div>
				<?php endif; ?>
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => __('Shop owner:', 'rehub-framework'));
		$instance = wp_parse_args( (array) $instance, $defaults ); ?>
		<p><em style="color:red;"><?php _e('Use only for vendor store widget area', 'rehub-framework');?></em></p>
		<p>
