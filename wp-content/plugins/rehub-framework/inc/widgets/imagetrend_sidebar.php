...
class rehub_postimagetrend_widget extends WP_Widget {
    ...
    function __construct() {
		$widget_ops = array( 'classname' => 'postimagetrend', 'description' => __('Widget that displays image grid posts. Use only in sidebar!', 'rehub-framework') );
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'rehub_postimagetrend' );
        parent::__construct('rehub_postimagetrend', __('ReHub: Image grid posts', 'rehub-framework'), $widget_ops, $control_ops );
    }
...
function widget( $args, $instance ) {
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => __('Trending', 'rehub-framework'), 'number' => 6, 'tags' => '', 'nohead' => true, 'two' => false);
		...
		<p>
			<label for="<?php echo ''.$this->get_field_id( 'nohead' ); ?>"><?php _e('Disable headings  ?', 'rehub-framework'); ?></label>
		...
		<p>
			<label for="<?php echo ''.$this->get_field_id( 'two' ); ?>"><?php _e('Two column?', 'rehub-framework'); ?></label>
