...
class rehub_posts_widget extends WP_Widget {
    ...
    function __construct() {
		$widget_ops = array( 'classname' => 'posts_widget', 'description' => __('A widget that displays custom posts list. Use only in sidebar!', 'rehub-framework') );
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'rehub_posts_widget' );
        parent::__construct( 'rehub_posts_widget', __('ReHub: Posts List', 'rehub-framework'), $widget_ops, $control_ops);
    }
...
function widget( $args, $instance ) {
	...
	<?php wp_reset_query(); ?>
	<?php else: ?><?php _e('No posts for this criteria.', 'rehub-framework'); ?>
	<?php endif; ?>
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => __('Latest Posts', 'rehub-framework'), 'number' => 5, 'categories' => '', 'sortby' => 'all_time', 'post_type' => 'all', 'dark' => '', 'cpt' => '');
		...
		<p>
		<label for="<?php echo ''.$this->get_field_id('categories'); ?>"><?php _e('Filter by Category:', 'rehub-framework'); ?></label> 
		<select id="<?php echo ''.$this->get_field_id('categories'); ?>" name="<?php echo ''.$this->get_field_name('categories'); ?>" class="widefat categories" style="width:100%;">
			<option value='all' <?php if ('all' == $instance['categories']) echo 'selected="selected"'; ?>><?php _e('All categories', 'rehub-framework'); ?></option>
		...
		<p>
		<label for="<?php echo ''.$this->get_field_id('sortby'); ?>"><?php _e('Posts sort by:', 'rehub-framework');?></label> 
		<select id="<?php echo ''.$this->get_field_id('sortby'); ?>" name="<?php echo ''.$this->get_field_name('sortby'); ?>" style="width:100%;">
			<option value='all_time' <?php if ( 'all_time' == $instance['sortby'] ) : echo 'selected="selected"'; endif; ?>><?php _e('all time', 'rehub-framework');?></option>
			<option value='this_week' <?php if ( 'this_week' == $instance['sortby'] ) : echo 'selected="selected"'; endif; ?>><?php _e('this week', 'rehub-framework');?></option>
			<option value='this_month' <?php if ( 'this_month' == $instance['sortby'] ) : echo 'selected="selected"'; endif; ?>><?php _e('this month', 'rehub-framework');?></option>
			<option value='three_month' <?php if ( 'three_month' == $instance['sortby'] ) : echo 'selected="selected"'; endif; ?>><?php _e('last 3 month', 'rehub-framework');?></option>
			<option value='random' <?php if ( 'random' == $instance['sortby'] ) : echo 'selected="selected"'; endif; ?>><?php _e('random', 'rehub-framework');?></option>
		</select>
		</p>

		<p>
			<label for="<?php echo ''.$this->get_field_id( 'number' ); ?>"><?php _e('Number of posts to show:', 'rehub-framework'); ?></label>
		...
		<p>
		<label for="<?php echo ''.$this->get_field_id('post_type'); ?>"><?php _e('Post Format:', 'rehub-framework');?></label> 
		<select id="<?php echo ''.$this->get_field_id('post_type'); ?>" name="<?php echo ''.$this->get_field_name('post_type'); ?>" style="width:100%;">
			<option value="all" <?php if ( 'all' == $instance['post_type'] ) : echo 'selected="selected"'; endif; ?>><?php _e('all', 'rehub-framework');?></option>
			<option value="regular" <?php if ( 'regular' == $instance['post_type'] ) : echo 'selected="selected"'; endif; ?>><?php _e('regular', 'rehub-framework');?></option>
			<option value="video" <?php if ( 'video' == $instance['post_type'] ) : echo 'selected="selected"'; endif; ?>><?php _e('video', 'rehub-framework');?></option>
			<option value="gallery" <?php if ( 'gallery' == $instance['post_type'] ) : echo 'selected="selected"'; endif; ?>><?php _e('gallery', 'rehub-framework');?></option>
			<option value="review" <?php if ( 'review' == $instance['post_type'] ) : echo 'selected="selected"'; endif; ?>><?php _e('review', 'rehub-framework');?></option>
			<option value="music" <?php if ( 'music' == $instance['post_type'] ) : echo 'selected="selected"'; endif; ?>><?php _e('music', 'rehub-framework');?></option>
		</select>
		</p>

		<p>
			<label for="<?php echo ''.$this->get_field_id( 'cpt' ); ?>"><?php _e('Post type (optional)', 'rehub-framework'); ?></label>
