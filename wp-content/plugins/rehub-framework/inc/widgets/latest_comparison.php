...
class rh_latest_compare_widget extends WP_Widget {
    ...
    function __construct() {
		$widget_ops = array( 'classname' => 'rh_latest_compare_widget top_offers', 'description' => __('Widget displays latest comparisons for woocommerce products, you must use shortcode [wpsm_woocharts] on comparison page to count products. Use only in sidebar!', 'rehub-framework') );
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'rh_latest_compare' );
        parent::__construct('rh_latest_compare', __('ReHub: Latest Woo Comparisons', 'rehub-framework'), $widget_ops, $control_ops );
    }
...
function widget( $args, $instance ) {
				...
				foreach ($comparedarray as $key => $value) {
					...
					echo '<div class="wpsm-versus-item">';
						...
						echo '<div class="vs-1-col vs-conttext rh_deal_block">';
							...
							echo '</a>'; 
							if($comparebtn){ 
                            	echo wpsm_comparison_button(array('class' => 'minicompare', 'id' => $value[0], 'label' => __('Compare', 'rehub-framework')));
                            } 							
						echo '</div>';
						...
						echo '<div class="vs-2-col vs-conttext rh_deal_block">';
							...
							echo '</a>';
							if($comparebtn){ 
                            	echo wpsm_comparison_button(array('class' => 'minicompare', 'id' => $value[1], 'label' => __('Compare', 'rehub-framework')));
                            } 							
						echo '</div>';						
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => __('Latest Comparisons', 'rehub-framework'), 'number' => 5);
		...
		<p>
			<label for="<?php echo ''.$this->get_field_id( 'number' ); ?>"><?php _e('Number of products (maximum is 10)', 'rehub-framework'); ?></label>
