...
class rehub_deal_daywoo_widget extends WP_Widget {
    ...
    function __construct() {
        $widget_ops = array( 'classname' => 'deal_daywoo woocommerce', 'description' => __('Displays "deal of the day" (WC Product or Post Offers). Use only in sidebar!', 'rehub-framework') );
        $control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'rehub_deal_daywoo' );
        parent::__construct( 'rehub_deal_daywoo', __( 'ReHub: Deal of day', 'rehub-framework' ), $widget_ops, $control_ops );
    }
...
function widget( $args, $instance ) {
				    ...
				    <figure class="position-relative">
						<?php
							if ( $_product->is_featured() ) :
								echo apply_filters( 'woocommerce_featured_flash', '<span class="onfeatured">' . __( 'Featured!', 'rehub-framework' ) . '</span>', $loop->post, $_product );
							endif; 
			        	...
			        	<?php if ( !empty($percentage) ) : ?>        
					        <div class="woo_spec_bar mt30 mb20">
					        	...
					        	<span class="stock-sold floatleft">
					        		<?php _e( 'Already Sold:', 'rehub-framework' );?> <strong><?php echo esc_html( $stock_sold ); ?></strong>
					        	</span>
					        	<span class="stock-available floatright">
					        		<?php _e( 'Available:', 'rehub-framework' );?> <strong><?php echo esc_html( $stock_available ); ?></strong>
					        	</span>
					        	</div>
	...
	<?php else: ?>
		<?php _e( 'No products for this criteria.', 'rehub-framework' );  ?>
	<?php endif; ?>
	...
	function form( $instance ) {
		...
		$defaults = array( 
			'title' => __( 'Deal of the day', 'rehub-framework' ),
			'dealtype' => 'product', 
			'dealid' => '', 
			'faketimer' => '', 
			'fakebar' => '', 
			'fakebar_sold' => 12,
			'fakebar_stock' => 16,
			'markettext' => __( 'Hurry Up! Offer ends soon.', 'rehub-framework' ),
			'carousel' => '',
			'autorotate' => '',
			'carouselnumber' => '3',
		);
		...
		<p>
			<label for="<?php echo ''.$this->get_field_id( 'dealtype' ); ?>"><?php _e('Post type:', 'rehub-framework'); ?></label>
			<select id="<?php echo ''.$this->get_field_id( 'dealtype' ); ?>" name="<?php echo ''.$this->get_field_name( 'dealtype' ); ?>" style="width:100%;">
				<option value="product" <?php if ( 'product' == $instance['dealtype'] ) : echo 'selected="selected"'; endif; ?>><?php _e('Product', 'rehub-framework'); ?></option>
				<option value="post" <?php if ( 'post' == $instance['dealtype'] ) : echo 'selected="selected"'; endif; ?>><?php _e('Post', 'rehub-framework'); ?></option>
			</select>
		</p>
		
		<p>
			<label for="<?php echo ''.$this->get_field_id( 'dealid' ); ?>"><?php _e('Ids of product to show:', 'rehub-framework'); ?></label>
			...
			<small><?php _e('By default, widget shows latest product which is on sale, you can specify product ID to overwrite this. You can set several ids if you enable carousel. Divide them by commas', 'rehub-framework'); ?></small>
		</p>

		<p>
			<label for="<?php echo ''.$this->get_field_id( 'faketimer' ); ?>"><?php _e('Set fake timer:', 'rehub-framework'); ?></label>
			...
			<br /><small><?php _e('By default, widget shows countdown base on Sale price dates of product. You can enable fake timer (always shows 12 hours)', 'rehub-framework'); ?></small>
		</p>						

		<p>
			<label for="<?php echo ''.$this->get_field_id( 'markettext' ); ?>"><?php _e('Add marketing text:', 'rehub-framework'); ?></label>
		...
		<p>
			<label for="<?php echo ''.$this->get_field_id( 'fakebar' ); ?>"><?php _e('Set fake sold bar:', 'rehub-framework'); ?></label>
			...
			<br>
			<label for="<?php echo ''.$this->get_field_id( 'fakebar_sold' ); ?>"><?php _e('Sold:', 'rehub-framework'); ?></label>
			...
			<label for="<?php echo ''.$this->get_field_id( 'fakebar_stock' ); ?>"><?php _e('In Stock:', 'rehub-framework'); ?></label>
			...
			<br /><small><?php _e('By default, widget shows real progress bar based on stock status, you can enable fake bar', 'rehub-framework'); ?></small>
		</p>
		<p>
			<label for="<?php echo ''.$this->get_field_id( 'carousel' ); ?>"><?php _e('Enable carousel?', 'rehub-framework'); ?></label>
		...
		<p>
			<label for="<?php echo ''.$this->get_field_id( 'autorotate' ); ?>"><?php _e('Enable Auto rotation?', 'rehub-framework'); ?></label>
		...
		<p>
			<label for="<?php echo ''.$this->get_field_id( 'carouselnumber' ); ?>"><?php _e('Number of posts in carousel', 'rehub-framework'); ?></label>
