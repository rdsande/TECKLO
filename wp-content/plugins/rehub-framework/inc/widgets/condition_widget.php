...
class rehub_conditional_widget extends WP_Widget {
    ...
    function __construct() {
		$widget_ops = array( 'classname' => 'conditional_widget_widget', 'description' => __('Widget allows to show content only on single pages.', 'rehub-framework') );
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'rehub_conditional_widget' );
        parent::__construct('rehub_conditional_widget', __('ReHub: Show only in singular pages', 'rehub-framework'), $widget_ops, $control_ops  );
    }
...
function widget( $args, $instance ) {
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo ''.$this->get_field_id( 'type' ); ?>"><?php _e('Type post type where to show content. Default is post.', 'rehub-framework'); ?></label>
		...
		<p>
			<label for="<?php echo ''.$this->get_field_id( 'postid' ); ?>"><?php _e('Type post ID if you want to have widget only there. You can set several ids with comma', 'rehub-framework'); ?></label>
		...
		<p>
			<label for="<?php echo ''.$this->get_field_id( 'reviewonly' ); ?>"><?php _e('Enable only in posts with review?', 'rehub-framework'); ?></label>
