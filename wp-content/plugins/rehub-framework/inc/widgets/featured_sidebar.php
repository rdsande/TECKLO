...
class rehub_featured_slider_widget extends WP_Widget {
    ...
    function __construct() {
        $widget_ops = array( 'classname' => 'featured_slider flexslider', 'description' => __('Widget that displays custom featured slider of posts or products. Use only in sidebar!', 'rehub-framework') );
        $control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'rehub_featured_slider' );
        parent::__construct('rehub_featured_slider', __('ReHub: Featured Slider', 'rehub-framework'), $widget_ops, $control_ops);
    }
...
function widget( $args, $instance ) {
					    ...
					    <figure class="centered_image_woo">
					        <a href="<?php echo esc_url($woolink) ;?>"<?php echo ''.$wootarget ;?>>
					            <?php if ( $product->is_featured() ) : ?>
					                    <?php echo apply_filters( 'woocommerce_featured_flash', '<span class="onfeatured">' . __( 'Featured!', 'rehub-framework' ) . '</span>', $post, $product ); ?>
					            <?php endif; ?>        
				...
				<?php else:?>
					<div class="wrap rehub-main-font">
						<a href="<?php the_permalink();?>" class="view-link">
							...
							<?php if(vp_metabox('rehub_post.rehub_framework_post_type') == 'review') :?><span class="score"><i><?php echo rehub_get_overall_score() ?></i><?php _e('SCORE', 'rehub-framework'); ?></span><?php endif;?>
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => __('Featured', 'rehub-framework'), 'number' => 3, 'tags' => '', 'post_type' => '');
		...
		<p>
			<label for="<?php echo ''.$this->get_field_id( 'post_type' ); ?>"><?php _e('Show woocommerce products', 'rehub-framework'); ?></label>
