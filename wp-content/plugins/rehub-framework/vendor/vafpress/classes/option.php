...
class VP_Option
{
	...
	public function __construct(array $configs)
	{
		...
		$configs = array_merge(array(
			...
			'layout'                => 'fixed',
			'page_title'            => __( 'Vafpress Options', 'rehub-framework' ),
			'menu_label'            => __( 'Vafpress Options', 'rehub-framework' ),
			'priority'              => 10,
		), $configs);
	...
	function ajax_import_option()
	{
		...
		if($result['status'])
		{
			...
			{
				$result['status']  = false;
				$result['message'] = __("Can not be empty", 'rehub-framework');
			}
			else
			{
				...
				{
					$result['status']  = false;
					$result['message'] = __("Invalid data", 'rehub-framework');
				}
			}
		}
	...
	function ajax_export_option()
	{
		...
		if($result['status'])
		{
			...
			$result = array(
				'status' => true,
				'message'=> __("Successful", 'rehub-framework'),
				'option' => $sr_options,
			);
		}
	...
	function vp_verify_nonce()
	{
		...
		{
			$result['status']  = true;
			$result['message'] = __("Successful", 'rehub-framework');	
		}
		else
		{
			$result['status']  = false;
			$result['message'] = __("Unverified Access", 'rehub-framework');
		}
		return $result;
	}
